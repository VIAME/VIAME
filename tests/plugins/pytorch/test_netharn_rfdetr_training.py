...
def test_launcher_training_settings(arch, normalization, clipping):
    # `wait()` returns the exit code, an int -- never None. Upstream's
    # f6d98fc4b made update_model raise unless it is 0, which is right,
    # and this mock returned None, which no real Popen does. Reported
    # upstream; fixed here so the check it added is exercised rather
    # than tripped over.
    ...
    # A successful run leaves a model behind, and f6d98fc4b made
    # update_model say so rather than report success with nothing to
    # show. These tests check the launch command of a run that worked,
    # so the stub is what a run that worked returns.
