...
/// Unit tests for the image_kernels kernels.
///
...
#include <image_kernels/statistics.h>
...
using namespace viame::image_kernels;
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, cast_pixel_truncates_and_does_not_clamp )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, round_pixel_rounds_half_away_from_zero )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, planes_to_grey_uses_bt709_weights )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, mean_over_planes_truncates_in_the_pixel_type )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, mean_over_planes_wraps_around_in_the_pixel_type )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, combine_channels_picks_luminance_only_for_three_planes )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, force_three_channels_repeats_plane_zero )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, scale_rounds_and_saturates_at_the_top )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, sample_and_sort_walks_every_pixel_of_a_single_plane )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, sample_and_sort_strides_over_all_planes )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, sample_and_sort_can_drop_the_extremes )
{
  auto const image = make_image< uint8_t >(
    4, 1, 1, { 0, 7, 9, 255 } );
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, percentiles_index_by_rounded_fraction_of_the_last_index )
{
  auto const image = make_image< uint8_t >(
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, percentile_stretch_maps_the_band_onto_the_output_range )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, percentile_stretch_clips_outside_the_band )
{
  auto const image = make_image< uint16_t >(
    5, 1, 1, { 100, 200, 300, 400, 500 } );
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, threshold_above_is_inclusive )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, threshold_percentile_cuts_at_the_sampled_value )
{
  auto const image = make_image< uint8_t >(
    5, 1, 1, { 10, 20, 30, 40, 50 } );
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, cumulative_average_is_the_mean_of_every_frame )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, cumulative_average_keeps_fractions_between_frames )
{
  frame_averager< uint8_t > averager( average_mode::cumulative );
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, cumulative_average_can_round_on_the_way_out )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, exponential_average_weights_the_new_frame )
{
  frame_averager< uint8_t > averager(
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, exponential_average_rejects_a_weight_outside_the_range )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, window_average_while_the_window_fills )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, window_average_subtracts_the_newest_frame_once_full )
{
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, variance_is_zero_on_the_first_frame )
{
  frame_averager< uint8_t > averager( average_mode::cumulative );
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, variance_multiplies_the_distance_before_and_after )
{
  frame_averager< uint8_t > averager( average_mode::cumulative );
  kv::image_of< double > variance;
...
// ----------------------------------------------------------------------------
TEST ( image_kernels, a_resolution_change_resets_the_average )
{
  frame_averager< uint8_t > averager( average_mode::cumulative );
