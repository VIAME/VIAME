...
::pair_stereo_tracks()
    : m_detection_pairing( new pair_stereo_detections() )
{
}
    ...
    const viame::timestamp& timestamp )
{
  ...
  for( const auto& track : tracks )
  {
    ...
    auto state = std::dynamic_pointer_cast< viame::object_track_state >( track->back() );
  ...
  if( filtered_tracks.size() != tracks_positions.size() )
  {
    VITAL_THROW( viame::invalid_data,
                 "Expected same tracks and position size. Got : " +
                 std::to_string( filtered_tracks.size() ) + " VS " +
...
viame::pair_stereo_tracks
::keep_right_tracks_in_current_frame(
    const std::vector< viame::track_sptr >& tracks,
    const viame::timestamp& timestamp )
{
  std::vector< viame::track_sptr > filtered_tracks;
...
viame::track_id_t
viame::pair_stereo_tracks
::last_left_right_track_id() const
{
  auto get_map_ids = []( const std::map< viame::track_id_t, viame::track_sptr >& map )
  {
    std::set< viame::track_id_t > track_ids;
...
{
  int frame_count{ -1 };
  viame::track_id_t right_id{ -1 };
};


std::tuple< std::vector< viame::track_sptr >, std::vector< viame::track_sptr > >
viame::pair_stereo_tracks
::get_left_right_tracks_with_pairing()
{
  std::vector< viame::track_sptr > left_tracks, right_tracks;
  ...
  const auto append_unprocessed_tracks = [](
      const std::map< viame::track_id_t, viame::track_sptr >& tracks_map,
      std::set< viame::track_id_t >& processed,
      std::vector< viame::track_sptr >& vect )
  {
...
std::tuple< std::set< viame::track_id_t >, std::set< viame::track_id_t > >
viame::pair_stereo_tracks
::select_most_likely_pairing( std::vector< viame::track_sptr >& left_tracks,
                              std::vector< viame::track_sptr >& right_tracks )
{
  std::map< viame::track_id_t, MostLikelyPair > most_likely_left_to_right_pair;
  ...
  std::set< viame::track_id_t > proc_left, proc_right;
...
std::tuple< std::set< viame::track_id_t >, std::set< viame::track_id_t > >
viame::pair_stereo_tracks
::split_paired_tracks_to_new_tracks(
    std::vector< viame::track_sptr >& left_tracks,
    std::vector< viame::track_sptr >& right_tracks )
{
...
void
viame::pair_stereo_tracks
::append_paired_frame( const viame::track_sptr& left_track,
                       const viame::track_sptr& right_track,
                       const viame::timestamp& timestamp )
{
...
void
viame::pair_stereo_tracks
::pair_left_right_tracks(
    const std::vector< viame::track_sptr >& left_tracks,
    const std::vector< viame::Detections3DPositions >& left_3d_pos,
    const std::vector< viame::track_sptr >& right_tracks,
    const viame::timestamp& timestamp )
{
  m_detection_pairing->m_verbose = m_verbose;
  ...
  std::vector< viame::track_sptr > filtered_left, filtered_right;
  std::vector< viame::detected_object_sptr > filtered_left_detections, filtered_right_detections;
  std::vector< viame::Detections3DPositions > filtered_3d_pos;

  auto get_current_track_detection = [&timestamp](
      const viame::track_sptr& track ) -> viame::detected_object_sptr
  {
    auto state = std::dynamic_pointer_cast< viame::object_track_state >( track->back() );
    if( track->last_frame() < timestamp.get_frame() || !state )
    {
  ...
  const auto pairings = m_detection_pairing->pair_left_right_detections(
    filtered_left_detections, filtered_3d_pos, filtered_right_detections );
...
std::vector< viame::track_sptr >
viame::pair_stereo_tracks
::filter_tracks_with_threshold( std::vector< viame::track_sptr > tracks ) const
{
  const auto is_outside_detection_number_threshold = [this]( const viame::track_sptr& track )
  {
    return ( ( int ) track->size() < m_min_detection_number_threshold ) ||
           ( ( int ) track->size() > m_max_detection_number_threshold );
  };

  const auto is_outside_detection_area_threshold = [this]( const viame::track_sptr& track )
  {
    double avg_area{};
    for( const auto& state : *track | viame::as_object_track )
    {
      if( state->detection() )
      {
...
inline std::string to_string( const std::map< size_t, viame::Pairing >& left_to_right_pairing )
{
  ...
  for( const auto& pair : left_to_right_pairing )
  {
    ...
    ss << "ID: " << pair.first << ", left: " << pair.second.left_right_id_pair.left_id
       << ", right: " << pair.second.left_right_id_pair.right_id
       << ", frames: " << print_frames << std::endl;
  }
  return ss.str();
}

std::vector< viame::pair_stereo_tracks::Range >
viame::pair_stereo_tracks
::create_split_ranges_from_track_pairs(
    const std::map< size_t, Pairing >& left_to_right_pairing ) const
{
  ...
  viame::frame_id_t last_pairings_frame_id{};
                                   ...
                                   const std::set< std::shared_ptr< Range > >& to_remove )
  {
    ...
    erase_if( pending_ranges,
              ...
              { return to_remove.find( range ) != std::end( to_remove ); } );
  };
  ...
  const auto create_range_from_pairing = [&]( const Pairing& pairing, viame::frame_id_t i_frame )
  {
  ...
  // For each frame
  for( viame::frame_id_t i_frame = 0; i_frame <= last_pairings_frame_id; i_frame++ )
  {
    // For each pairing
    for( const auto& pairing : left_to_right_pairing )
    {
...
::split_ranges_to_tracks( const std::vector< Range >& ranges,
                          ...
                          std::vector< viame::track_sptr >& right_tracks )
{
  const auto split_track = []( const viame::track_sptr& track, const Range& range )
  {
    auto new_track = viame::track::create();
