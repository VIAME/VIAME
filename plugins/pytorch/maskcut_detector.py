...
class MaskCutDetector(ImageObjectDetector):
    ...
    _options = [
        ...
        # Dino ViT
        _Option(
            "_pretrained_path", "pretrained_path", "", str, "path to pretrained model"
        ),
        _Option(
            "_vit_arch", "vit_arch", "small", str, "which architecture"
        ),  # ['small', 'base']
        _Option(
            "_vit_feat", "vit_feat", "k", str, "which features"
        ),  # ['k', 'q', 'v', 'kqv']
    ...
    def check_configuration(self, cfg):
        ...
        if self._vit_arch not in valid_vit_archs:
            print(
                f"ViT arch {self._vit_arch} is not valid, must be one of {valid_vit_archs}"
            )
            return False
        ...
        if self._vit_feat not in valid_vit_feats:
            print(
                f"ViT feat {self._vit_feat} is not valid, must be one of {valid_vit_feats}"
            )
            return False
        ...
        if self._patch_size not in valid_patch_sizes:
            print(
                f"Patch size {self._patch_size} is not valid, must be one of {valid_patch_sizes}"
            )
            return False
    ...
    def detect(self, image_data):
        ...
        # preprocess image
        fixed_size = 480
        I_new = input_image.resize(
            (int(fixed_size), int(fixed_size)), PIL.Image.LANCZOS
        )
        ...
        for idx, bipartition in enumerate(bipartitions):
            ...
            annotation_info = create_annotation_info(
                idx, 0, category_info, pseudo_mask.astype(np.uint8), input_image_size
            )
            ...
            [tl_x, tl_y, w, h] = np.array(annotation_info["bbox"]).astype(
                np.int32
            )  # xywh
            ...
            detected_object = DetectedObject(
                bounding_box, class_confidence, detected_object_type
            )

            poly = kwimage.Mask(
                annotation_info["segmentation"], "bytes_rle"
            ).to_multi_polygon()
...
def __vital_algorithm_register__():
    ...
    register_vital_algorithm(
        MaskCutDetector, "maskcut", "maskcut detection inference routine"
    )
