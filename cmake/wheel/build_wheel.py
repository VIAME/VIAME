...
# Tool launchers
#
# `bin/viame` is an ELF that links `libpython3.X.so.1.0`, and its RUNPATH
# reaches the environment's `lib` and the nvidia wheels but not the
# interpreter's own library directory. Whether it starts therefore depends on
# whether libpython happens to be somewhere the loader already looks.
#
# On a distribution python it is -- Ubuntu puts libpython3.10 in
# /usr/lib/x86_64-linux-gnu, which is in the ldconfig cache. On a standalone
# interpreter, which is what pyenv, conda, uv and python.org's own builds all
# are, it is not, and the tool dies before `main` with
#
#   viame: error while loading shared libraries: libpython3.11.so.1.0
#
# RUNPATH cannot fix this: the path is only known once you know which
# interpreter is running, which is not a build-time fact. So the wheel ships
# the ELF out of the way and puts a python script on the PATH in its place.
# The script runs in the interpreter, asks it where its own library lives,
...
Generated by cmake/wheel/build_wheel.py. See the note beside LAUNCHER there
for why this is not simply the binary.
"""
import os
import sys
import sysconfig
...
def main():
    real = os.path.join(sys.prefix, "libexec", "{name}")

    if not os.path.exists(real):
        sys.exit("{name}: {{}} is missing; the install is incomplete".format(real))
    ...
    if paths:
        ...
        merged = [p for p in paths if p not in existing.split(os.pathsep)]
        if merged:
    ...
    try:
        ...
        sys.exit("{name}: cannot run {{}}: {{}}".format(real, e))
...
detection, tracking, classification, size measurement, stereo/multi-camera
processing, search and training, assembled into configurable pipelines.
    ...
    try:
        with zipfile.ZipFile(whl, "w", zipfile.ZIP_DEFLATED) as z:
            ...
            for name in (args.launcher or []):
                ...
                if dest in chosen:
                    raise SystemExit(
                        f"build_wheel: --launcher {name} collides with a "
                        f"contents rule that already packs {dest}. The binary "
                        f"belongs under {{data}}/libexec/, not in scripts.")
                launchers[dest] = LAUNCHER.replace("{name}", name)
            ...
            for dest, text in {**launchers, **extras}.items():
                data = text.encode()
                if dest in launchers:
                    # 0o755 through the scripts scheme, as pip would for a
                    # console script; a launcher that is not executable is
                    # the same as no launcher.
                    zi = zipfile.ZipInfo(dest, date_time=(1980, 1, 1, 0, 0, 0))
                    zi.external_attr = (0o100755 << 16)   # S_IFREG | 0755
...
def main(argv=None):
    ...
    p.add_argument("--launcher", action="append", metavar="NAME",
                   help="put a python launcher for this tool on the PATH, "
                        "and expect the real binary at "
                        "{data}/libexec/NAME. Repeatable. See the note "
                        "beside LAUNCHER for why the ELF cannot go on the "
                        "PATH directly.")
