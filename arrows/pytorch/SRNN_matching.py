...
class TargetRNNDataLoader(data.Dataset):
    ...
    def _make_dataset(self):
        ...
        for t in range(self._tracks_num):
            ...
            if cur_track.active_flag is True and cur_track.updated_flag is False:
                for ts in range(self._track_states_num):
                    if _rnnType is self._rnnType:
                        dis_vec = np.asarray(cur_track[-1].bbox_center) - np.asarray(self._track_state_list[ts].bbox_center) 
                        dis = np.linalg.norm(dis_vec)
    
                        # in the search area, we prepare data and calculate the similarity score
                        if dis < self._track_search_threshold:
                            cur_data_item = tuple()
...
class SRNN_matching(object):
    def __init__(self, targetRNN_full_model_path, targetRNN_AIM_V_model_path, batch_size=256):
        ...
        self._targetRNN_AIM_V_model = TargetLSTM(model_list=model_list).cuda()
    ...
    def _est_similarity(self, loader, rnnType):
        ...
        for (app_f_list, app_target_f, motion_f_list, motion_target_f, interaction_f_list, interaction_target_f, t, ts) in loader:
            ...
            elif rnnType == RnnType.Target_RNN_AIM_V:
