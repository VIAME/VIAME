...
::load_( std::string const& filename ) const
{
  ...
  else
  {
    ...
    {
      if( pair.left == c_from_camera && pair.right == c_to_camera )
      {
        matrix = forward_matrix( pair, filename );
        found = true;
        break;
      }
      if( pair.left == c_to_camera && pair.right == c_from_camera )
      {
    ...
    if( !found )
    {
      throw std::runtime_error( "No pair registering " + c_from_camera +
        " onto " + c_to_camera + " in: " + filename );
    }
  }
         ...
         kwiver::vital::transform_2d_sptr data ) const
{
  ...
  std::string const right =
    c_to_camera.empty() ? "right" : c_to_camera;
