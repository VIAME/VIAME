...
``configs/add-ons/dino3/stereo_measure_current_annots_ncc_dino.pipe``. Per source
keypoint it does:
  ...
  2. DINO stage: extract dense DINOv2 ViT features for both images, bilinearly
     ...
     ``library/measurement/dino/dino_matcher.py``: forward_features -> x_norm_patchtokens
...
try:  # imported as a package member (the plugin loader's path)
    from viame.measurement.onnx.epipolar_matcher import (
...
class EpipolarDinoCropMeasurer(EpipolarDinoCropMatcher):
    ...
    def forward(self, left_rgb, right_rgb, points_left,
                ...
                min_depth, max_depth):
        try:
            from viame.measurement.onnx.triangulate import triangulate_fast_torch
        except ImportError:
