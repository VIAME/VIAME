...
::stitch(
  ...
  viame::image_container_sptr ) const
{
  ...
  std::vector< viame::track_sptr > all_tracks = input->tracks();
  viame::frame_id_t frame_to_stitch = frame_number - c_new_shot_length + 1;
  ...
  viame::frame_id_t frame_to_test = frame_to_stitch + 1;
  ...
  viame::frame_id_t last_frame_to_test = 0;
  ...
  auto stitch_frame_set = std::make_shared< viame::feature_track_set >(
    input->active_tracks( frame_to_stitch ) );

  for(; frame_to_test > last_frame_to_test; frame_to_test-- )
  {
    auto test_frame_set = std::make_shared< viame::feature_track_set >(
      input->active_tracks( frame_to_test ) );

    // run matcher alg
    viame::match_set_sptr mset = c_feature_matcher->match(
        ...
        static_cast< size_t >( c_percent_match_req * total_features ) )
    {
      ...
      std::vector< viame::track_sptr > test_frame_trks =
        test_frame_set->tracks();
      std::vector< viame::track_sptr > stitch_frame_trks =
        stitch_frame_set->tracks();
      std::vector< viame::match > matches = mset->matches();
