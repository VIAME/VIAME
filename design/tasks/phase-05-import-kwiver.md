...
- There must be exactly one `vital` in the process. The first attempt at this task followed the note that used to be here -- copy vital, point VIAME at the copy, leave kwiver building its own for sprokit and arrows -- and both copies then get loaded: kwiver's through sprokit and the arrows, the copy through VIAME's own libraries. The code is identical and the namespace and include guards are the same, so it compiles, links, and runs; what it does not do is exit. Every `kwiver runner` process aborts in `__run_exit_handlers` with a double free, deterministically, and removing the copied library from the install makes it stop. So kwiver's sprokit and arrows have to move onto the copy in the same step that builds it, which means P5-T02, P5-T03 and P5-T04 land together: copy all three groups, rewrite the includes in what is left of kwiver, and build kwiver's remnants against `viame_algorithm_framework` with `vital/` not built at all.
- `core_types` and `algorithm_framework` are one shared library, not two. The two directories name each other -- the types need configuration, logging and errors, and the algorithm interfaces are written in terms of the types -- and CMake allows a cycle only between static libraries. Kwiver cut the same code into eight libraries to keep the graph acyclic; which cuts are worth keeping is phase 8's question. `viame_core_types` is an alias so that a consumer can still say which half it wants.
- The generated headers (`vital_config.h`, `version.h`, `kwiver-include-paths.h`, the export headers) are generated into the build tree from kwiver's own templates, with kwiver's macro and guard names kept. One of them carries absolute paths from the build and has no business in the source tree.
- The python bindings move with P5-T05 rather than here. Two `kwiver.vital.types` extension modules in one interpreter is a pybind11 duplicate-registration error, so they cannot be moved while kwiver's own python package is still installed.
Done when:
- `library/`, `plugins/` and `tools/` compile against the copied vital; no `libvital*.so` is loaded by anything; BASELINE passes and a pipeline exits zero.
