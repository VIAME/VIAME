...
class ClfDataset(torch_data.Dataset):
    ...
    def input_id(self):
        ...
        # imgaug_json_id() falls back to repr() for the wrapper, which embeds the
        # object address and changes the resume directory on every launch
        depends = [
    ...
    def make_loader(self, batch_size=16, num_batches='auto', num_workers=0,
        ...
        if balance == 'sequential':
            from viame.object_detectors.netharn.netharn.data.batch_samplers import (
            ...
            from viame.object_detectors.netharn.netharn.data.batch_samplers import BalancedBatchSampler
            index_to_cid = [
