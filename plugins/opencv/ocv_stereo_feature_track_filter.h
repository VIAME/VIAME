#ifndef VIAME_OCV_STEREO_FEATURE_TRACK_FILTER_H
#define VIAME_OCV_STEREO_FEATURE_TRACK_FILTER_H
...
struct VIAME_OPENCV_EXPORT StereoPointCoordinates {
  ...
  static StereoPointCoordinates from_features(const FrameFeatureTrackStates &features, const Landmarks &landmarks);
};
...
public:
  ...
  select_points_maximizing_variance(const StereoPointCoordinates &coordinates, size_t frame_count_threshold);
  ...
  /// quality.
  static StereoPointCoordinates
  select_frames(const FeatureTracks &features, const Landmarks &landmarks, size_t frame_count_threshold);
...
private:
  ...
  static FrameFeatureTrackStates remove_frames_without_corresponding_left_right_match(FrameFeatureTrackStates features);

  /// @brief Returns the maximum id in the input feature track vector given a max function
  static int64_t max_feature_id(const FeatureTracks &feature_tracks,
  ...
  get_destination_extent(const cv::Point3f &world_pt, const std::array<cv::Point3f, 4> &world_points_corner_values);

  static std::set<size_t>
  select_frames_in_cluster(const cv::Mat &frames_matrix, const cv::Mat &cluster_labels, const cv::Mat &cluster_centers);

  static StereoPointCoordinates
  filter_points_in_kept_frames(const StereoPointCoordinates &coordinates, const std::set<size_t> &selected_frame_idx);
};
}

#endif //VIAME_OCV_STEREO_FEATURE_TRACK_FILTER_H
