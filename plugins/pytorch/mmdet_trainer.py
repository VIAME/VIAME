...
class MMDetTrainer(TrainDetector):
    ...
    def set_configuration(self, cfg_in):
        ...
        if self._train_directory is not None:
            self._train_config = os.path.join(self._train_directory, self._train_config)
    ...
    def internal_update(self):
        ...
        train_dataset = CustomDataset(self._training_store, self._cfg.train_pipeline)

        if self._validate:
            validation_dataset = CustomDataset(
                self._validation_store, self._cfg.test_pipeline
            )
    ...
    def insert_training_params(self, input_cfg, output_cfg):
        ...
        repl_strs = [
            ["[-CLASS_COUNT_INSERT-]", str(len(self._categories) + 1)],
...
def __vital_algorithm_register__():
    ...
    register_vital_algorithm(
        MMDetTrainer, "mmdet", "PyTorch MMDetection training routine"
    )
