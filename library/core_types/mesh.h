...
public:
  ...
  virtual void
  append( const mesh_vertex_array_base& verts )
  {
  ...
  bool
  has_normals() const { return !normals_.empty(); }

  /// Set the vertex normals
  void
  set_normals( const std::vector< vector_3d >& n )
  {
  ...
  vector_3d&
  normal( unsigned int v ) { return normals_[ v ]; }
  const vector_3d&
  normal( unsigned int v ) const { return normals_[ v ]; }

  /// Access the normals
  const std::vector< vector_3d >&
  normals() const { return normals_; }
...
public:
  ...
  size() const { return static_cast< unsigned int >( verts_.size() ); }
  ...
  /// Access a vertex coordinate by vertex index and coordinate index
  virtual double
  operator()( unsigned int v, unsigned int i ) const
  {
    return verts_[ v ][ i ];
  }
  ...
  append( const mesh_vertex_array_base& verts )
  {
    ...
    const mesh_vertex_array< d >& v =
      static_cast< const mesh_vertex_array< d >& >( verts );
  ...
  void
  push_back( const vert_t& v ) { verts_.push_back( v ); }

  /// Access a vertex
  vert_t&
  operator[]( unsigned int v ) { return verts_[ v ]; }
  const vert_t&
  operator[]( unsigned int v ) const { return verts_[ v ]; }
  ...
  iterator
  begin() { return verts_.begin(); }
  const_iterator
  begin() const { return verts_.begin(); }

  iterator
  end() { return verts_.end(); }
  const_iterator
  end() const { return verts_.end(); }
};
...
public:
  ...
  /// Accessor
  unsigned int
  operator[]( unsigned int i ) const { return verts_[ i ]; }
  unsigned int&
  operator[]( unsigned int i ) { return verts_[ i ]; }
...
class VIAME_CORE_TYPES_EXPORT mesh_face_array_base
{
public:
  /// Destructor
  ...
  virtual void
  flip_orientation( unsigned int f )
  {
    if( has_normals() )
    {
      normals_[ f ] *= -1;
  ...
  vector_3d&
  normal( unsigned int f ) { return normals_[ f ]; }
  const vector_3d&
  normal( unsigned int f ) const { return normals_[ f ]; }
  ...
  bool
  has_groups() const { return !groups_.empty(); }
...
class VIAME_CORE_TYPES_EXPORT mesh_face_array : public mesh_face_array_base
{
...
public:
  ...
  /// Constructor (from an initializer list)
  mesh_face_array(
    const std::initializer_list< std::vector< unsigned int > >& faces )
    : faces_( faces ) {}
  ...
  /// otherwise
  virtual unsigned int
  regularity() const { return 0; }

  /// returns the number of faces
  virtual unsigned int
  size() const { return static_cast< unsigned int >( faces_.size() ); }

  /// returns the number of vertices in face \param f
  virtual unsigned int
  num_verts( unsigned int f ) const
  {
    return static_cast< unsigned int >( faces_[ f ].size() );
  }

  /// Access a vertex index by face index and within-face index
  virtual unsigned int
  operator()( unsigned int f, unsigned int i ) const
  {
    return faces_[ f ][ i ];
  }
  ...
  void
  push_back( const std::vector< unsigned int >& f ) { faces_.push_back( f ); }

  /// Add a face to the array
  template < unsigned int s >
  void
  push_back( const mesh_regular_face< s >& f )
  {
  ...
  operator[]( unsigned int f ) { return faces_[ f ]; }
  const std::vector< unsigned int >&
  operator[]( unsigned int f ) const { return faces_[ f ]; }
};
...
public:
    ...
    unsigned int ind_shift = 0 )
  {
    ...
    const unsigned int new_begin =
      static_cast< unsigned int >( faces_.size() );
  ...
  /// Add a face to the array
  void
  push_back( const mesh_regular_face< s >& f ) { faces_.push_back( f ); }

  /// Access face \param f
  mesh_regular_face< s >&
  operator[]( unsigned int f ) { return faces_[ f ]; }
  const mesh_regular_face< s >&
  operator[]( unsigned int f ) const { return faces_[ f ]; }

  // --------------------------------------------------------------------------
  // Face Iterators
  typedef typename std::vector< mesh_regular_face< s > >::iterator iterator;
  typedef typename std::vector< mesh_regular_face< s > >::const_iterator
    const_iterator;

  iterator
  begin() { return faces_.begin(); }
  const_iterator
  begin() const { return faces_.begin(); }

  iterator
  end() { return faces_.end(); }
  const_iterator
  end() const { return faces_.end(); }
};
...
class VIAME_CORE_TYPES_EXPORT mesh_half_edge
{
...
public:
  ...
  /// Equality operator
  bool
  operator==( mesh_half_edge const& other ) const
  {
  ...
  unsigned int
  next_index() const { return next_; }
  /// return the pair half-edge index
  unsigned int
  pair_index() const { return edge_ ^ 1; }

  /// return the index of the full edge
  unsigned int
  edge_index() const { return edge_ >> 1; }
  /// return the index of the half-edge
  unsigned int
  half_edge_index() const { return edge_; }

  /// return the vertex index
  unsigned int
  vert_index() const { return vert_; }
  /// return the face index
  unsigned int
  face_index() const { return face_; }

  bool
  is_boundary() const { return face_ == mesh_invalid_idx; }
...
class VIAME_CORE_TYPES_EXPORT mesh_half_edge_set
{
public:
  /// Default Constructor
  mesh_half_edge_set() {}
  ...
  mesh_half_edge_set(
    const std::vector< std::vector< unsigned int > >& face_list );
  ...
  operator[]( unsigned int i ) const { return half_edges_[ i ]; }
  /// Access by index
  mesh_half_edge&
  operator[]( unsigned int i )  { return half_edges_[ i ]; }

  /// number of half edges
  unsigned int
  size() const { return static_cast< unsigned int >( half_edges_.size() ); }
  ...
  public:
    ...
    /// Assignment
    f_iterator&
    operator=( const f_iterator& other )
    {
      if( this != &other )
      {
    ...
    bool
    operator==( const f_iterator& other ) const
    {
    ...
    bool
    operator==( const f_const_iterator& other ) const
    {
      return this->half_edge_index_ == other.half_edge_index_ &&
             &( this->edge_set_ ) == &( other.edge_set_ );
    }
  ...
  public:
    ...
    /// Assignment
    f_const_iterator&
    operator=( const f_const_iterator& other )
    {
      if( this != &other )
      {
        assert( &edge_set_ == &other.edge_set_ );
  ...
  public:
    ...
    /// Assignment
    v_iterator&
    operator=( const v_iterator& other )
    {
      if( this != &other )
      {
        assert( &edge_set_ == &other.edge_set_ );
    ...
    bool
    operator==( const v_iterator& other ) const
    {
      return this->half_edge_index_ == other.half_edge_index_ &&
             &( this->edge_set_ ) == &( other.edge_set_ );
    }
    ...
    bool
    operator==( const v_const_iterator& other ) const
    {
      return this->half_edge_index_ == other.half_edge_index_ &&
             &( this->edge_set_ ) == &( other.edge_set_ );
    }
  ...
  public:
    ...
    /// Assignment
    v_const_iterator&
    operator=( const v_const_iterator& other )
    {
      if( this != &other )
      {
        assert( &edge_set_ == &other.edge_set_ );
  ...
  f_const_iterator
  face_begin( unsigned int f ) const
  {
    return f_const_iterator( face_to_he_[ f ], *this );
  }

  f_iterator
  face_begin( unsigned int f )
  {
    return f_iterator( face_to_he_[ f ], *this );
  }

  /// Access a vertex iterator for vertex \param v
  v_const_iterator
  vertex_begin( unsigned int v ) const
  {
    return v_const_iterator( vert_to_he_[ v ], *this );
  }

  v_iterator
  vertex_begin( unsigned int v )
  {
    return v_iterator( vert_to_he_[ v ], *this );
  }
...
class VIAME_CORE_TYPES_EXPORT mesh
{
public:
  /// Default Constructor
  ...
  /// Return the number of vertices
  unsigned int
  num_verts() const { return verts_->size(); }

  /// Return the number of faces
  unsigned int
  num_faces() const { return faces_->size(); }

  /// Return the number of edges
  unsigned int
  num_edges() const { return half_edges_.size() / 2; }
  ...
  bool
  is_init() const { return verts_.get() && faces_.get(); }

  /// Access the vector of vertices
  const mesh_vertex_array_base&
  vertices() const { return *verts_; }
  mesh_vertex_array_base&
  vertices() { return *verts_; }

  /// Access the vector of vertices cast to a dimension
  template < unsigned int d >
  const mesh_vertex_array< d >&
  vertices() const
  {
  ...
  /// Set the vertices
  void
  set_vertices( std::unique_ptr< mesh_vertex_array_base > verts )
  {
    verts_ = std::move( verts );
  }
  ...
  /// Set the faces
  void
  set_faces( std::unique_ptr< mesh_face_array_base > faces )
  {
    faces_ = std::move( faces );
  }
  ...
  bool
  has_half_edges() const { return half_edges_.size() > 0; }

  /// Return the half edge set
  const mesh_half_edge_set&
  half_edges() const { return half_edges_; }
  ...
  void
  remove_edge_graph() { half_edges_.clear(); }
  ...
  tex_coord_type
  has_tex_coords() const { return tex_coord_status_; }

  /// Return the texture coordinates
  const std::vector< vector_2d >&
  tex_coords() const { return tex_coords_; }
  ...
  void
  set_tex_source( const std::string ts ) { tex_source_ = ts; }

  const std::string&
  tex_source() const { return tex_source_; }

  /// Return a vector indicating which faces have texture
  const std::vector< bool >&
  valid_tex_faces() const { return valid_tex_faces_; }

  /// Set the vector indicating which faces have texture
  void set_valid_tex_faces( const std::vector< bool >& valid );

  /// Label all faces with positive (counter clockwise orientation) area as
  /// valid
  ///
