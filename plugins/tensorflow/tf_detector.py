...
class TFDetector(ImageObjectDetector):
    ...
    def detect(self, in_img_c):
        ...
        if self.norm_image_type and self.norm_image_type != "none":
            ...
            bottom, top = self.get_scaling_values(
                self.norm_image_type, in_img, image_height
            )
        ...
        logger.debug(
            "Done running detector in %s", humanfriendly.format_timespan(elapsed)
        )
        ...
        for i in range(0, len(scores)):
            if scores[i] >= self.confidence_thresh:
                ...
                obj = DetectedObject(
                    BoundingBoxD(xmin, ymin, xmax, ymax), scores[i], dot
                )
                detections.add(obj)
    ...
    def lin_normalize_image(self, image_array, bottom=None, top=None):
        ...
        scaled_image = np.floor(scaled_image * 255).astype(
            np.uint8
        )  # Map to [0, 2^8 - 1]
...
def __vital_algorithm_register__():
    ...
    register_vital_algorithm(
        TFDetector, "tensorflow", "Tensorflow detector testing routine"
    )
