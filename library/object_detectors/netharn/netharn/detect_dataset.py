...
from viame.object_detectors.netharn.netharn.data.data_containers import ItemContainer
...
class DetectFitDataset(torch.utils.data.Dataset):
    ...
    def make_loader(self, batch_size=16, num_workers=0, shuffle=False,
        ...
        if balance != 'tfidf':
            ...
            if shuffle:
                from viame.object_detectors.netharn.netharn.data.batch_samplers import PatchedRandomSampler
                item_sampler = PatchedRandomSampler(self, num_samples=num_samples)
            else:
                from viame.object_detectors.netharn.netharn.data.batch_samplers import SubsetSampler
        ...
        if balance == 'tfidf':
            ...
            #     item_sampler, batch_size=batch_size, drop_last=drop_last)
            from viame.object_detectors.netharn.netharn.data.batch_samplers import PatchedBatchSampler
