 ...
 * and more.
 *
 * \section design_type_safety Type Safety
 *
 * The codebase strives for type safety where possible. This is achieved by
 * using \c typedef to rename types. When applicable, \c typedef types also
 * expose objects through only a \c shared_ptr to prevent unintentional deep
 ...
 * immutable through their respective \c typedef and can only be created with
 * static methods of the respective class which enforce that they can only be
 * constructed in specific ways.
 *
 * \section design_introspection Introspection
 *
 ...
 *
 * \section design_thread_safety Thread safety
 *
 ...
 *
 * \section design_error_handling Error Handling
 *
 ...
 *
 * \section design_control_flow Control Flow
 *
 ...
 *
 * \subsection design_data_flow_ports Ports
 *
 ...
 *
 * \subsection design_data_flow_packets Packets
 *
 ...
 * inputs:
 *
 * \li If a required input is complete, then the current process' computation is
 *     considered to be complete as well.
 * \li Otherwise, if a required input is an error message, then the current
 *     process' computation is considered an error due to an error as input
 *     (following the GIGO principle).
 * \li Otherwise, if a required input is empty, then the current process'
 *     computation is considered empty (the computation is missing data and
 *     cannot be completed).
 * \li Then, since all of the required inputs are available, the stamps are
 ...
 *
 * \section design_pipeline_execution Pipeline Execution
 *
