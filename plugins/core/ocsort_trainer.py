...
class OCSORTTrainer(TrainTracker):
    ...
    def update_model(self):
        ...
        params_file = os.path.join(self._output_directory, "ocsort_params.json")
    ...
    def _save_final_config(self, params):
        ...
        if not self._pipeline_template:
            config_content = f"""# OC-SORT tracker configuration (estimated from training data)
# Generated by ocsort_trainer
...
process tracker
  ...
  use_vdc = true
  vdc_weight = {params['vdc_weight']:.3f}
  use_oru = true
...
"""
            output_pipeline = os.path.join(
                self._output_directory, "ocsort_tracker.pipe"
            )
        ...
        if os.path.exists(self._pipeline_template):
            ...
            for key, value in params.items():
                ...
                if isinstance(value, float):
                    pipeline = pipeline.replace(placeholder, f"{value:.3f}")
                else:
                    pipeline = pipeline.replace(placeholder, str(value))
