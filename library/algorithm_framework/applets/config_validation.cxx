  ...
  kv::logger_handle_t logger )
{
  ...
  {
    LOG_ERROR(
      logger, "Configuration value for " << name
  ...
  bool test_write )
{
  ...
  return validate_optional_output_file(
    name, config, logger,
      ...
      config.get_value< std::string >( name ) != "" )
  {
    ...
    if( !ST::FileIsDirectory( parent_dir ) )
    {
      if( make_directory )
      {
        ...
        {
          LOG_ERROR(
            logger, "unable to create directory " << parent_dir
                                                  <<
      ...
      {
        LOG_ERROR(
          logger, "directory " << parent_dir << " does not exist"
                               << " for configuration option " << name);
        return false;
      }
    }
    ...
    if( test_write )
    {
      ...
      {
        LOG_ERROR(
          logger, "Could not open file " << path << " for writing "
                                         <<
  ...
  bool make_directory )
{
      ...
      config.get_value< std::string >( name ) != "" )
  {
    ...
    if( !ST::FileIsDirectory( path ) )
    {
      ...
      LOG_ERROR(
        logger, path << " is not a valid directory for "
