...
PYBIND11_MODULE( pipeline, m )
{
  ...
  class_< sprokit::pipeline, sprokit::pipeline_t >(
    ...
    .def(
      "connection_to_addr", &sprokit::pipeline::connection_to_addr,
      arg( "name" ), arg( "port" ),
      "Return the address for the port that is connected upstream of a port." )
    .def(
      "upstream_for_process", &sprokit::pipeline::upstream_for_process,
      ( arg( "name" ) ),
      "Return all processes upstream of the given process." )
    .def(
      "upstream_for_port", &sprokit::pipeline::upstream_for_port,
      arg( "name" ), arg( "port" ),
      "Return the process upstream of the given port." )
    .def(
      "downstream_for_process", &sprokit::pipeline::downstream_for_process,
      ( arg( "name" ) ),
      "Return all processes downstream of the given process." )
    .def(
      "downstream_for_port", &sprokit::pipeline::downstream_for_port,
      arg( "name" ), arg( "port" ),
      "Return the processes downstream of the given port." )
    .def(
      "sender_for_port", &sprokit::pipeline::sender_for_port,
      arg( "name" ), arg( "port" ),
    ...
    .def(
      "edge_for_connection", &sprokit::pipeline::edge_for_connection,
      arg( "upstream_name" ), arg( "upstream_port" ), arg( "downstream_name" ),
      arg( "downstream_port" ),
      "Returns the edge for the connection." )
    .def(
      "input_edges_for_process", &sprokit::pipeline::input_edges_for_process,
      ( arg( "name" ) ),
      "Return the edges that are sending to the given process." )
    .def(
      "input_edge_for_port", &sprokit::pipeline::input_edge_for_port,
      arg( "name" ), arg( "port" ),
      return_value_policy::reference,
      "Return the edge that is sending to the given port." )
    .def(
      "output_edges_for_process", &sprokit::pipeline::output_edges_for_process,
      ( arg( "name" ) ),
      "Return the edges that are receiving data from the given process." )
    .def(
      "output_edges_for_port", &sprokit::pipeline::output_edges_for_port,
      arg( "name" ), arg( "port" ),
...
connections_from_addr(
  ...
  ::sprokit::process::port_t const& port )
{
  ::sprokit::process::port_addrs_t pair_addrs =
...
receivers_for_port(
  ...
  ::sprokit::process::port_t const& port )
{
  ::sprokit::process::port_addrs_t pair_addrs = self.receivers_for_port(
    name,
    port );
  std::vector< wrap_port_addr > wrap_addrs;
  for( unsigned int idx = 0; idx < pair_addrs.size(); idx++ )
