...
python.exe "%VIAME_INSTALL%\configs\process_video.py" --init -l "%INPUT_LIST%" -id "%INPUT_DETECTIONS%" -ifrate %INPUT_FRAME_RATE% -frate %PROCESS_FRAME_RATE% -p pipelines\index_existing_detections.res.pipe --build-index --ball-tree -install "%VIAME_INSTALL%"
