...
# The entry point groups were named `kwiver.*` until P11-T02 renamed the
# package. A package built against the old names is still installed out
# there and still declares them, and an entry point group that nothing reads
# fails silently -- the plugin is simply absent, with no error anywhere. So
# both names are read for the one release the `kwiver` compatibility package
# lasts; see `library/compat/kwiver`.
_get_ns_entrypoints = get_ns_entrypoints


def get_ns_entrypoints(ns: str) -> Iterable["metadata.EntryPoint"]:
    found = list(_get_ns_entrypoints(ns))
    if ns.startswith("viame."):
        found += list(_get_ns_entrypoints("kwiver." + ns[len("viame."):]))
    return found
...
def import_via_entrypoint_extensions(entrypoint_ns: str) -> Set[ModuleType]:
    ...
    For example, as a single specification string::
        ...
        ...
        entry_points={
            "viame.python_plugins": "my_package = my_package.plugins"
        ]
        ...
...
def _get_concrete_pluggable_types() -> List[Type[Pluggable]]:
    ...
    try:
        from viame.algo.algorithm_factory import get_registered_algorithms
...
# so. Four of the sixteen named packages this tree installs under no option,
# and they are not here. Two were `kwiver.arrows.core` and
# `kwiver.arrows.python`, whose sources P11-T02b deleted outright: nothing
# had built them since they were commented out for kwiver-v2. The other two
# were `viame.processes.pytorch` and `viame.pipeline.tests.
# processes`. Those are the spellings from before phase 11 renamed the
# package to `viame`, which is why they read as they do.
#
