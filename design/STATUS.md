...
| P5-T02 | Copy `vital` into `core_types` and `algorithm_framework` | P5-T01 | in-progress | 080b61004 | The copy itself is done and committed: 408 files into `library/core_types` and `library/algorithm_framework`, kwiversys, cereal, rapidjson and cxxopts into `third_party/`, includes rewritten, by `design/scripts/import_kwiver_vital.py` so it is repeatable rather than a one-off. It compiled, linked and ran: the whole tree built against the copy, BASELINE passed, and pipelines produced correct output. What it would not do is exit. The task text said to leave kwiver building its own vital for sprokit and arrows, "temporary duplication for one task"; both copies then load, and every `kwiver runner` process aborts in `__run_exit_handlers` with a double free, eight times out of eight. Removing the copied library from the install makes it stop, three times out of three, which is what pins the cause on the duplication rather than on anything in the copy. The code is identical and so are the namespace and the include guards, which is why it builds and runs at all; what is not shared is the static state, and two sets of it do not survive teardown. So the build switch is reverted and the tree is back on kwiver's vital, with the copied sources and the import script kept in place for the next attempt. The task text now says what that attempt has to be: P5-T02, P5-T03 and P5-T04 land together, with kwiver's remnants built against the copy and `vital/` not built at all. Two other findings are recorded there: the two directories have to be one shared library, because they name each other and CMake allows a cycle only between static libraries; and the python bindings cannot move until kwiver's python package goes, because two `kwiver.vital.types` extension modules in one interpreter is a pybind11 duplicate-registration error |
