...
#include <viame/algorithm_framework/config/token_type_config.h>
...
public:
  priv()
    : m_line_number( 0 ),
      ...
      m_logger( viame::get_logger( "viame.config_parser" ) )
  {
    ...
    while( true )
    {
      ...
      if( token.value == "include" )
      {
        ...
        const std::string exp_filename =
          m_token_expander.expand_token( m_token_line );
        ...
        LOG_DEBUG(
          m_logger, "Including file \"" << resolv_filename << "\" at "
      ...
      if( token.value == "block" )
      {
        ...
        if( token.type != token_t::TK_WORD )
        {
          ...
          LOG_ERROR(
            m_logger, "Invalid syntax in line \"" << m_last_line <<
      ...
      while( token.type == token_t::TK_FLAG )
      {
        ...
        {
          LOG_ERROR(
            m_logger, "Unrecognized flags: \"" << token.value
                                               << "\" at " << file_path <<
              ":" << m_line_number );
          m_parse_error = true;
        }
...
{
  m_priv->m_search_path.insert(
    m_priv->m_search_path.end(),
