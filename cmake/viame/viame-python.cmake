...
# in any `.py` under `python/` or `library/` is `@template@`, in
...
#+
# `viame_create_python_init`, unless the source directory has an
# `__init__.py` of its own -- in which case the glob already installs it and
# a stub would take its place. Installing it is still this function's job:
# `viame_add_python_module` installs the built copy at the same path, so the
# two `install( FILES )` agree.
#-
function( _viame_package_init directory modpath )
  if( EXISTS "${directory}/__init__.py" )
    return()
  endif()
...
function( viame_add_python_package modpath )
  ...
  # An `__init__.py` for every level that has none of its own.
  #
  # Only for the levels that have none: `viame_create_python_init` writes its
  # stub at **configure** time, and the glob's copy of a real `__init__.py`
  # is an `add_custom_command` that runs at **build** time and only when the
  # source is newer than the output. Write the stub over a package that has
  # a real one and the copy never runs -- the package installs as
  # `# Generated by VIAME`, declaring nothing, and the plugins it was meant
  # to declare are silently absent. That is how it went wrong the first time.
  _viame_package_init( "${PKG_DIRECTORY}" "${modpath}" )
  ...
  foreach( subpackage IN LISTS subpackages )
    ...
    foreach( part IN LISTS parts )
      ...
      _viame_package_init( "${PKG_DIRECTORY}/${walked}" "${modpath}/${walked}" )
    endforeach()
  endforeach()
endfunction()
