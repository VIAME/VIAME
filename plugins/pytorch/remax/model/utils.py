...
def sigmoid_asl_loss(inputs, target, num_boxes, eps=0.1, gamma_pos=0, gamma_neg=4):
    num_classes = inputs.size()[-1]
    log_preds = nn.LogSoftmax(dim=-1)(inputs)
    targets_classes = torch.zeros_like(inputs).scatter_(1, target.long(), 1)
    ...
    asymmetric_w = torch.pow(1 - xs_pos - xs_neg, gamma_pos * targets + gamma_neg * anti_targets)
    log_preds = log_preds * asymmetric_w

    if eps > 0:  # label smoothing
        targets_classes = targets_classes.mul(1 - eps).add(eps / num_classes)
