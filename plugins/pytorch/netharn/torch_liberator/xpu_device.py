...
class XPU(ub.NiceRepr):
    ...
    def memory(self):
        ...
        Example:
            ...
            >>> from torch_liberator.xpu_device import *  # NOQA
    ...
    def load(xpu, fpath):
        ...
        Example:
            ...
            >>> dpath = ub.Path.appdir('torch_liberator/doctest/xpu').ensuredir()
        ...
        try:
            return torch.load(fpath, map_location=xpu._map_location, weights_only=False)
        except Exception:
...
def gpu_info(new_mode=True, respect_visible_devices=True):
    ...
    Example:
        >>> # xdoctest: +REQUIRES(--cuda)
        >>> from torch_liberator.xpu_device import gpu_info
        >>> gpus = gpu_info()
