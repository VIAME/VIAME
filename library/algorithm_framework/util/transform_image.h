...
/// not have the
/// correct dimensions its memory will be reallocated to match the dimensions of
/// the input image.
...
{
  T2
  operator()( T1 const& v ) const { return static_cast< T2 >( v ); }
};
...
{
  bool
  operator()( T1 const& v ) const { return v != T1( 0 ); }
};

/// Static cast an image of one type to that of another type
template < typename T1, typename T2 >
void
cast_image( image_of< T1 > const& img_in, image_of< T2 >& img_out )
{
...
template < typename T >
void
cast_image( image const& img_in, image_of< T >& img_out )
{
