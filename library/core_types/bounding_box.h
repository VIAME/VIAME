...
private:
  ...
  friend bounding_box< T1 >& translate(
    bounding_box< T1 >& bbox,
    typename bounding_box< T1 >::vector_type const& pt );

  template < typename T1 >
  friend bounding_box< T1 > scale(
    bounding_box< T1 > const& bbox,
    double scale_factor );

  template < typename T2 >
  friend bounding_box< T2 > intersection(
  ...
  double scale_factor )
{
  return bounding_box< T >(
    ( bbox.upper_left().template cast< double >() *
      scale_factor ).template cast< T >(),
    ( bbox.lower_right().template cast< double >() *
      scale_factor ).template cast< T >() );
}
