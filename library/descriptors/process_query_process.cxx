...
create_config_trait( descriptor_index_dir, std::string, "",
  "Folder of per-video descriptor bundles: for every indexed video a "
  "<name>_descriptors.npy (N x D float32 or float64) with a <name>_uids.txt "
  "listing one uid per row. Every bundle in the folder is loaded." );
create_config_trait( descriptor_npy_postfix, std::string, "_descriptors.npy",
  "Postfix of the per-video descriptor array files in descriptor_index_dir" );
create_config_trait( descriptor_uids_postfix, std::string, "_uids.txt",
...
create_config_trait( lsh_hashes_postfix, std::string, "_hashes.npy",
  "Postfix of the per-video LSH hash code files (uint8, N x bit_length) in "
  "descriptor_index_dir; each pairs with the <name>_uids.txt of its bundle. "
  "When descriptor_index_dir holds such files they are used instead of "
  "lsh_hash_codes_file / lsh_hash_uids_file." );
create_config_trait( lsh_hash_codes_file, std::string, "database/ITQ/lsh_hash_codes.npy",
  ...
  "Generated by viame index build." );
...
public:
  ...
  std::string m_descriptor_index_dir;
  std::string m_descriptor_npy_postfix;
  std::string m_descriptor_uids_postfix;
  std::string m_lsh_hashes_postfix;
  ...
  void load_descriptor_index()
  {
    ...
    {
      load_descriptor_index_from_dir();
      return;
    }
  ...
  /// Load every per-video bundle (<name>_descriptors.npy + <name>_uids.txt)
  /// in the index folder. A folder with no bundles yields an empty index,
  /// which is valid (nothing indexed yet) rather than an error.
  void load_descriptor_index_from_dir()
  {
    if( !std::filesystem::is_directory( m_descriptor_index_dir ) )
    {
      throw std::runtime_error( "Descriptor index folder does not exist: " +
        m_descriptor_index_dir );
    }

    std::vector< std::string > array_files;
    for( auto const& entry : std::filesystem::directory_iterator( m_descriptor_index_dir ) )
    {
      if( entry.is_regular_file() &&
          numpy_array_reader::ends_with( entry.path().filename().string(),
                                         m_descriptor_npy_postfix ) )
      {
        array_files.push_back( entry.path().string() );
      }
    }
    std::sort( array_files.begin(), array_files.end() );

    for( auto const& array_file : array_files )
    {
      std::string uid_file =
        array_file.substr( 0, array_file.size() - m_descriptor_npy_postfix.size() )
        + m_descriptor_uids_postfix;

      std::vector< double > data;
      std::vector< size_t > shape;
      if( !numpy_array_reader::read_float64_array( array_file, data, shape ) ||
          shape.size() != 2 )
      {
        throw std::runtime_error( "Unable to read descriptor array (expected a "
          "2-D float32/float64 .npy): " + array_file );
      }
      std::vector< std::string > uids;
      if( !numpy_array_reader::read_uid_list( uid_file, uids ) )
      {
        throw std::runtime_error( "Missing uid list for " + array_file + ": " + uid_file );
      }
      if( uids.size() != shape[0] )
      {
        throw std::runtime_error( "Descriptor bundle mismatch: " + array_file +
          " has " + std::to_string( shape[0] ) + " rows but " + uid_file +
          " lists " + std::to_string( uids.size() ) + " uids" );
      }

      size_t const dim = shape[1];
      for( size_t row = 0; row < shape[0]; ++row )
      {
        m_descriptor_index[ uids[row] ] = std::vector< double >(
          data.begin() + row * dim, data.begin() + ( row + 1 ) * dim );
      }

      LOG_INFO( m_logger, "Loaded " << shape[0] << " descriptors ("
        << dim << "-d) from " << array_file );
    }
...
::_configure()
{
  ...
  d->m_descriptor_index_dir = config_value_using_trait( descriptor_index_dir );
  d->m_descriptor_npy_postfix = config_value_using_trait( descriptor_npy_postfix );
  d->m_descriptor_uids_postfix = config_value_using_trait( descriptor_uids_postfix );
  d->m_lsh_hashes_postfix = config_value_using_trait( lsh_hashes_postfix );
  ...
  {
    // Per-video hash files beside the descriptor bundles take precedence
    // over the single global hash table.
    if( !d->m_descriptor_index_dir.empty() )
    {
      d->m_lsh_loaded = d->m_lsh_index.load_from_dir(
        d->m_descriptor_index_dir,
        d->m_lsh_hashes_postfix,
        d->m_descriptor_uids_postfix,
    ...
    else
    {
      throw std::runtime_error(
        ...
        "are missing or invalid. Check that per-video <name>" +
        d->m_lsh_hashes_postfix + " files exist in " + d->m_descriptor_index_dir +
        " (with the ITQ model files below), or that the following files exist:\n"
        "  - " + d->m_lsh_hash_codes_file + "\n"
        "  - " + d->m_lsh_hash_uids_file + "\n"
        "  - " + d->m_itq_mean_vec_file + "\n"
        "  - " + d->m_itq_rotation_file + "\n"
        "These files are generated by viame index build."
...
::make_config()
{
  declare_config_using_trait( descriptor_index_dir );
  declare_config_using_trait( descriptor_npy_postfix );
  declare_config_using_trait( descriptor_uids_postfix );
  ...
  declare_config_using_trait( lsh_hashes_postfix );
