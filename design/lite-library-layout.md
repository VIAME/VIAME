...
sideways. Three of `plugins/core`'s helpers were moved down in P2-T04 for
...
| `python/kwiver/vital/` until P11, then alongside the C++ | `python/kwiver/vital/{types,algo(generated, replaced in P8),config,util,modules,plugin_management}` -- the bindings keep their package tree while the package is still called `kwiver`: 48 of the 67 files under `vital/types` share a name with the C++ file they bind, and every level of the package needs its own `__init__.py`, so they cannot sit beside it |
...
| `python/kwiver/sprokit/` until P11, then alongside the C++ | `python/kwiver/sprokit/{pipeline,pipeline_util,processes/kwiver_process,schedulers/pythread_per_process,adapters,util}`, for the same reason |
...
| Destination | Files |
|---|---|
| `utilities/` | `utilities_file`, `utilities_image`, `utilities_tracks`, `utilities_target_clfr`, `manipulate_pipelines`, `applet_attributes.h`, `python_script_applet`, `utils.py`, `vital_registration.py`; `plugins/compat.py`, `plugins/types.py` |
