...
public:
  ...
  kv::feature_set_sptr frame_features(
    kv::frame_id_t offset = -1 ) const override;
  kv::descriptor_set_sptr frame_descriptors(
    kv::frame_id_t offset = -1 ) const override;
  std::vector< kv::feature_track_state_sptr > frame_feature_track_states(
    kv::frame_id_t offset = -1 ) const override;
...
PYBIND11_MODULE( feature_track_set, m )
{
  ...
  py::class_< feat_track_state, kwiver::vital::track_state,
    ...
    .def(
      py::init(
        [](kv::frame_id_t frame, kv::feature_sptr const& feature,
           kv::descriptor_sptr const& descriptor){
          return std::shared_ptr< feat_track_state >(
            new feat_track_state(
              frame, feature, descriptor ) );
        } ) )
    .def(
      "clone", ( [](feat_track_state& self){
  ...
  py::class_< feat_track_set, kwiver::vital::track_set,
    ...
    .def(
      "frame_features", &feat_track_set::frame_features,
      py::arg( "offset" ) = -1 )
    .def(
      "frame_descriptors", &feat_track_set::frame_descriptors,
      py::arg( "offset" ) = -1 )
    .def(
      "frame_feature_track_states", &feat_track_set::frame_feature_track_states,
      py::arg( "offset" ) = -1 )
