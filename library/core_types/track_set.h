...
typedef std::map< frame_id_t,
  track_set_frame_data_sptr > track_set_frame_data_map_t;
...
public:
  ...
  ///
  /// Inactive tracks are any tracks which do not contain a state on the target
  /// frame.
  ///
  /// \param [in] offset the frame offset for selecting the active frame.
  ///                    Positive number are absolute frame numbers while
  ...
  ///                    the default.
  ///
  /// \returns a vector of tracks that is the subset of tracks that are
  /// inactive.
  ...
  ///
  /// New tracks include any tracks with a first track state on the target
  /// frame.
  ///
  /// \param[in] offset the frame offset for selecting the active frame.
  ///                   Positive number are absolute frame numbers while
  ...
  virtual std::vector< track_sptr > new_tracks(
    frame_id_t offset = -1 ) const = 0;
  ...
  ///                   the default.
  ///
  /// \returns a vector of tracks containing all terminated tracks for the given
  /// frame.
  ...
  /// frames.
  ///
  /// The percentage of tracks successfully tracked between frames is defined as
  /// the
  /// number of tracks which have a track state on both frames, divided by the
  /// total
  ...
  virtual track_set_frame_data_sptr frame_data(
    frame_id_t offset = -1 ) const = 0;
...
typedef std::shared_ptr< track_set_implementation >
  track_set_implementation_sptr;
...
/// bridge design pattern.  This pattern allows multiple back end
/// implementations that
/// store and index track data in different ways.  Each back end can be combined
/// with
