...
tmux send-keys -t $SESSION:0 "pipeline_runner --pipe ${SCRIPT_DIR}/test_zmq_send.pipe" C-m
...
tmux send-keys -t 1 "pipeline_runner --pipe ${SCRIPT_DIR}/test_zmq_send2.pipe" C-m

sleep 1
echo "$(date) Preparing Receiver..."
tmux send-keys -t 2 "cd ${SCRIPT_DIR}" C-m
tmux send-keys -t 2 "source ${START_SCRIPT}" C-m
tmux send-keys -t 2 "pipeline_runner --pipe ${SCRIPT_DIR}/test_zmq_recv2.pipe"
