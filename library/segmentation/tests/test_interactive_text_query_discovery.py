...
pytest.importorskip("viame.algo")
from viame.segmentation import interactive_segmentation as seg  # noqa: E402
...
def test_add_on_text_query_config_is_found_when_the_default_names_none(config_dir):
    conf = config_dir / "interactive_segmenter_default.conf"
    try:
        kwiver.PerformTextQuery.create("sam3")
    except Exception:
        pytest.skip("sam3 text query plugin not built")
    (config_dir / "interactive_text_query_default.conf").write_text("# core placeholder\n")
    (config_dir / "interactive_text_query_sam3.conf").write_text(
        "block perform_text_query\n  type = sam3\n  sam3:device = cpu\nendblock\n"
    )
    assert seg.load_text_query_algo_from_config([str(conf)]) is not None
