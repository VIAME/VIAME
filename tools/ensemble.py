...
def train_stacker( frames, ann, id_to_label, args, method, weights,
                   sigma, iou_thr, n_models, fitted_model=None ):
...
def write_pipe_config( args, best, label_dic, pseudo_dic, pseudo_ind,
  ...
  with open( args.output_config, 'w' ) as fout:
    fout.write('# Optimization seed: %d; held-out fraction: %g\n' % (args.seed, args.validation_fraction))
...
def split_sequences(count, fraction, seed):
  if not np.isfinite(fraction) or not 0 <= fraction < 1:
    raise ValueError("validation-fraction must be finite and in [0, 1)")
  if count < 2 or fraction == 0:
    return list(range(count)), []
  size = min(count - 1, max(1, int(round(count * fraction))))
  validation = set(np.random.default_rng(seed).permutation(count)[:size].tolist())
  return [i for i in range(count) if i not in validation], sorted(validation)
...
def main():
  ...
  parser.add_argument('--validation-fraction', type=float, default=0.2,
    help='Fraction of whole sequences held out from calibration and optimization; 0 disables it')
  args = parser.parse_args()
  try:
    train_indices, val_indices = split_sequences(len(args.truth), args.validation_fraction, args.seed)
  except ValueError as exc:
    parser.error(str(exc))
  split_info = {'seed': args.seed, 'training_sequences': [args.truth[i] for i in train_indices],
                'validation_sequences': [args.truth[i] for i in val_indices]}
  if not val_indices:
    print('No held-out sequences: reported scores measure optimization data only.')
  else:
    print('Holding out %d sequence(s) from optimization.' % len(val_indices))
  ...
  # Class names are schema metadata; include held-out-only categories so
  # their missed objects still contribute to held-out recall.
  if label_dic is None:
    names = {d[5] for truth in seq_truth for dets in truth.values() for d in dets}
    names.update(d[5] for sequence in seq_computed for model in sequence for dets in model.values() for d in dets)
    label_dic = {'background': 0}
    label_dic.update({name: i + 1 for i, name in enumerate(sorted(names - {'background'}))})
  val_truth = [seq_truth[i] for i in val_indices]
  val_computed = [seq_computed[i] for i in val_indices]
  seq_truth = [seq_truth[i] for i in train_indices]
  seq_computed = [seq_computed[i] for i in train_indices]
  ...
  if args.calibrate:
    ...
    apply_calibration_to_dets( val_computed, tables )
  ...
  val_frames, val_ann, _, _, _ = build_frames(
    val_truth, val_computed, args, label_dic, pseudo_dic, pseudo_ind, need_dists)

  print( "\nOptimization-data baseline mAP@{:g}{}:".format( args.map_iou,
  ...
  if val_indices:
    validation_map = evaluate(val_frames, val_ann, id_to_label, args.map_iou,
      best_method, best[1], best[2], args.skip_box_thr, best[3])[0]
    validation_baselines = {'detector_' + str(m + 1): evaluate_baseline(
      val_frames, val_ann, id_to_label, args.map_iou, [m])[0] for m in range(n_models)}
    print('Held-out validation mAP: {:.4f}'.format(validation_map))
  ...
  if args.stack:
    ...
    if stack_model is None:
      ...
      selection_baseline = best[0]
      if val_indices:
        _, stack_map, _ = train_stacker(val_frames, val_ann, id_to_label, args,
          best_method, best[1], best[3], best[2], n_models, fitted_model=stack_model)
        selection_baseline = validation_map
        print('  held-out re-scored mAP: {:.4f}'.format(stack_map))
      if stack_map > selection_baseline:
        rescore_file = args.output_rescore
      else:
        print( "  (no improvement over fusion alone; not referenced in "
          "the output pipe config)" )

  print( "\nBest configuration (selected on optimization data):" )
  ...
  with open(args.output_config + ".split.json", "w") as stream:
    json.dump(split_info, stream, indent=2)

  if args.output_json:
    output = {
      'split': split_info,
      'score_scope': 'optimization',
      'validation_map': validation_map,
      'validation_baselines': validation_baselines,
      'rescored_map_scope': 'validation' if val_indices else 'optimization',
      'baselines': baselines,
