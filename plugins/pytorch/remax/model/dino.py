...
from .utils import sigmoid_focal_loss, sigmoid_asl_loss, MLP
...
class DINO(nn.Module):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        outputs_feats = torch.stack(hs)

        #for layer_cls_embed, layer_hs in zip(self.class_embed, hs):
        #    tmp = layer_cls_embed(layer_hs)
        #    pdb.set_trace()
        if self.dn_number > 0 and dn_meta is not None:
            outputs_class, outputs_coord_list, outputs_feats = \
                dn_post_process(outputs_class, outputs_coord_list, outputs_feats,
                                dn_meta,self.aux_loss,self._set_aux_loss)
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord_list[-1], 'pred_feats': outputs_feats[-1]}
        if self.aux_loss:
        ...
        if hs_enc is not None:
            ...
            if hs_enc.shape[0] > 1:
                ...
                for layer_id, (layer_box_embed, layer_class_embed, layer_hs_enc, layer_ref_enc) in enumerate(zip(self.enc_bbox_embed, self.enc_class_embed, hs_enc[:-1], ref_enc[:-1])):
                    ...
                    layer_enc_outputs_feats = layer_hs_enc
                    enc_outputs_coord.append(layer_enc_outputs_coord)
                    enc_outputs_class.append(layer_enc_outputs_class)
                    enc_outputs_feats.append(layer_enc_outputs_feats)
                #pdb.set_trace()
                ...
                out['enc_outputs'] = [
                    {'pred_logits': a, 'pred_boxes': b, 'pred_feats': c} for a, b, c in zip(enc_outputs_class, enc_outputs_coord, enc_outputs_feats)
                ]
        #pdb.set_trace()
        out['dn_meta'] = dn_meta
...
class SetCriterion(nn.Module):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        ...
        loss_ce = sigmoid_asl_loss(src_logits, target_classes_onehot, num_boxes) * src_logits.shape[1]
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        aa = sigmoid_focal_loss(src_masks, target_masks, num_boxes)
        bb = sigmoid_asl_loss(src_masks, target_masks, num_boxes)
        pdb.set_trace()
        losses = {
            ###"loss_mask": sigmoid_focal_loss(src_masks, target_masks, num_boxes),
            "loss_mask": sigmoid_asl_loss(src_masks, target_masks, num_boxes),
...
class PostProcess(nn.Module):
    ...
    def forward(self, outputs, target_sizes, not_to_xyxy=False, test=False):
        ...
        out_logits, out_bbox, out_feats = outputs['pred_logits'], outputs['pred_boxes'], outputs['pred_feats']
        print(out_logits, target_sizes)
        ...
        probs = torch.gather(prob, 1, topk_boxes.unsqueeze(-1).repeat(1,1,out_logits.shape[2])) ##
        # and from relative [0, 1] to absolute [0, height] coordinates
        img_h, img_w = target_sizes.unbind(1)
        scale_fct = torch.stack([img_w, img_h, img_w, img_h], dim=1)
        boxes = boxes * scale_fct[:, None, :]
        feats = torch.gather(out_feats, 1, topk_boxes.unsqueeze(-1).repeat(1,1,out_feats.shape[2]))

        if self.nms_iou_threshold > 0:
            item_indices = [nms(b, s, iou_threshold=self.nms_iou_threshold) for b,s in zip(boxes, scores)]
            # import ipdb; ipdb.set_trace()
            results = [{'scores': s[i], 'labels': l[i], 'boxes': b[i], 'logits': p[i], 'feats': f[i]} for s, l, b, f, i in zip(scores, labels, boxes, probs, feats, item_indices)]
        else:
            results = [{'scores': s, 'labels': l, 'boxes': b, 'logits': p, 'feats': f} for s, l, b, p, f in zip(scores, labels, boxes, probs, feats)]
