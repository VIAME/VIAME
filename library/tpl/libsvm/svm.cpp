...
private:
  ...
  double kernel_nmi(int i, int j) const
  {
    return nmi(x[i],x[j]);
  }
 ...
 gamma(param.gamma), coef0(param.coef0)
{
  switch(kernel_type)
  {
    ...
    case PRECOMPUTED:
      ...
      break;
    case HISTOGRAM:
      kernel_function = &Kernel::kernel_histogram;
      break;
    case NMI:
      kernel_function = &Kernel::kernel_nmi;
      break;
  }
...
double Kernel::histogram_intersection(const svm_node *px, const svm_node *py)
{
  double sum = 0;
  while(px->index != -1 && py->index != -1)
  {
...
double Kernel::nmi(const svm_node *px, const svm_node *py)
{
  double sumx = 0;
  double sumy = 0;
  int scale= 2000;
  ...
  while(px->index != -1 && py->index != -1)
  {
    if(px->index == py->index)
    {
      vx.push_back( px->value );
      vy.push_back( py->value );
...
//  printf("len(vx) = %d \t len(vy) = %d \n", vx.size(), vy.size());
  ...
  float *proby = new float[scale2];
  memset( proby, 0, sizeof(float)*scale2 );

  float *probxy = new float[scale2*scale2];
  memset( probxy, 0, sizeof(float)*scale2*scale2);

  int len=vx.size();
  float delta = 1.0/(float)len;
  for(int i=0; i<len; i++)
  {
    int x=(int)(vx[i]*sumx);
    int y=(int)(vy[i]*sumy);
...
//  printf("hy=%f\n", hy);
  float hxy = entropy(probxy, scale*scale);
//  printf("hxy=%f\n", hxy);
...
float Kernel::entropy(float *vec, int elements)
{
  float ent=0.0;
        ...
        const svm_parameter& param)
{
  switch(param.kernel_type)
  {
    case NMI:
      return nmi(x,y);
    case HISTOGRAM:
      return histogram_intersection(x,y);
    case LINEAR:
      return dot(x,y);
    case POLY:
...
double binary_predict_values(const svm_model *model, const svm_node *x, double* dec_values)
{
  if(model->param.svm_type == ONE_CLASS ||
     model->param.svm_type == EPSILON_SVR ||
     model->param.svm_type == NU_SVR)
...
double binary_predict(const svm_model *model, const svm_node *x)
{
  int nr_class = model->nr_class;
  double *dec_values;
  if(model->param.svm_type == ONE_CLASS ||
...
double binary_predict_probability(
  const svm_model *model, const svm_node *x, double *prob_estimates)
{
  if ((model->param.svm_type == C_SVC || model->param.svm_type == NU_SVC) &&
      model->probA!=NULL && model->probB!=NULL)
  {
    ...
    int nr_class = model->nr_class;
    double *dec_values = Malloc(double, nr_class*(nr_class-1)/2);
    binary_predict_values(model, x, dec_values);
...
// Stratified cross validation
void X_cross_validation(const svm_problem *prob, const svm_parameter *param, int nr_fold, double *target)
{
  int i;
  int *fold_start = Malloc(int,nr_fold+1);
  int l = prob->l;
  ...
  for(i=0;i<nr_fold;i++)
  {
    ...
    else
    {
      for(j=begin;j<end;j++)
      {
        target[perm[j]] = svm_predict(submodel,prob->x[perm[j]]);
        //////
        dec_values[perm[j]] = binary_predict(submodel, prob->x[perm[j]]);
      }
    }
    svm_free_and_destroy_model(&submodel);
    free(subprob.x);
