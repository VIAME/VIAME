...
import warnings
import kwcoco
from viame.object_detectors.netharn.netharn import torch_liberator
from viame.object_detectors.netharn.netharn.data.channel_spec import ChannelSpec
from viame.object_detectors.netharn.netharn.data.data_containers import ContainerXPU
import os
...
def setup_module_aliases():
    ...
    packages, which have now been merged into 'viame.object_detectors.netharn.netharn'. This
    ...
    Will resolve to the corresponding modules under viame.object_detectors.netharn.netharn.
    ...
    # netharn -> viame.object_detectors.netharn.netharn
    # bioharn -> viame.object_detectors.netharn.netharn (bioharn was merged into netharn)
    alias_mappings = {
        # Top-level aliases
        'netharn': nh,
        'bioharn': nh,
...
def _ensure_upgraded_model(deployed_fpath):
    ...
    """
    # Set up module aliases for backwards compatibility with old models
    # that use 'import netharn' or 'import bioharn' instead of 'viame.object_detectors.netharn.netharn'
            ...
            if hasattr(predictor.model.module, 'detector'):
                ...
                try:
                    ...
                    try:
                        ...
                        from viame.object_detectors.netharn.netharn.data.data_containers import BatchContainer
...
def _coerce_sampler(config):
    from viame.object_detectors.netharn.netharn import bio_util as util
