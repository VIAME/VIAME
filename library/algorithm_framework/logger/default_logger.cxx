...
public:
  /// CTOR
  default_logger(
    logger_ns::logger_factory_default* p,
  ...
  virtual bool
  is_fatal_enabled() const { return m_logLevel <= kwiver_logger::LEVEL_FATAL;  }

  virtual bool
  is_error_enabled() const { return m_logLevel <= kwiver_logger::LEVEL_ERROR;  }

  virtual bool
  is_warn_enabled()  const { return m_logLevel <= kwiver_logger::LEVEL_WARN;  }

  virtual bool
  is_info_enabled()  const { return m_logLevel <= kwiver_logger::LEVEL_INFO;  }

  virtual bool
  is_debug_enabled() const { return m_logLevel <= kwiver_logger::LEVEL_DEBUG;  }

  virtual bool
  is_trace_enabled() const { return m_logLevel <= kwiver_logger::LEVEL_TRACE;  }

  virtual void set_level( log_level_t lev ) { m_logLevel = lev; }

  virtual log_level_t
  get_level() const { return m_logLevel; }
...
::get_logger( std::string const& name )
{
  ...
  std::map< std::string,
    logger_handle_t >::iterator it = m_active_loggers.find( name );
