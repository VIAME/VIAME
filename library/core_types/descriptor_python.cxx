  ...
  py::slice slice, py::object val_obj )
{
  ...
  catch( ... )
  {
    ...
    for( size_t idx = start; idx < stop; idx += step )
    {
      data[ idx ] = val[ idx ]; // if there's an out of bounds here, python will
                                // throw an exception
    }
  }
}
...
template < typename T >
void
bind_descriptor( py::module& m, std::string&& typestr )
{
  ...
  py::class_< kwiver::vital::descriptor_dynamic< T >, kwiver::vital::descriptor,
    ...
    pyclass_name.c_str() )
    .def(
    "__setitem__", &set_slice< T >,
    py::arg( "slice" ), py::arg( "value" ) )
    .def(
      "__getitem__", &get_slice< T >,
      py::arg( "slice" ) )
    .def(
      "__setitem__", &set_index< T >,
      py::arg( "index" ), py::arg( "value" ) )
    .def(
      "__getitem__", &get_index< T >,
      py::arg( "index" ) );
}
