...
public:
  ...
  virtual vector_2d
  distort( const vector_2d& norm_pt ) const { return norm_pt; }

  /// Unmap distorted normalized coordinates into normalized coordinates
  ///
  ///  The default implementation is the identity transformation (no distortion)
  virtual vector_2d
  undistort( const vector_2d& dist_pt ) const { return dist_pt; }
  ...
  /// domain
  virtual bool
  is_map_valid( [[maybe_unused]] const vector_2d& norm_pt ) const
  {
    return true;
  }
...
public:
  ...
  explicit simple_camera_intrinsics(
    const matrix_3x3d& K,
    const vector_t& d = vector_t() );

  /// Access the focal length
  virtual double
  focal_length() const { return focal_length_; }
  /// Access the principal point
  virtual vector_2d
  principal_point() const { return principal_point_; }
  /// Access the aspect ratio
  virtual double
  aspect_ratio() const { return aspect_ratio_; }
  /// Access the skew
  virtual double
  ...
  dist_coeffs() const
  {
    return std::vector< double >(
      dist_coeffs_.data(),
      dist_coeffs_.data() + dist_coeffs_.size() );
  }

  /// Access the maximum distortion radius
  double
  max_distort_radius() const { return std::sqrt( max_distort_radius_sq_ ); }

  /// Access the focal length
  const double&
  get_focal_length() const { return focal_length_; }
  /// Access the principal point
  const vector_2d&
  get_principal_point() const { return principal_point_; }
  /// Access the aspect ratio
  const double&
  get_aspect_ratio() const { return aspect_ratio_; }
  /// Access the skew
  const double&
  ...
  /// Access the distortion coefficients
  const vector_t&
  get_dist_coeffs() const { return dist_coeffs_; }
  /// Access the maximum distortion radius squared
  const double&
  get_max_distort_radius_sq() const { return max_distort_radius_sq_; }

  /// Set the focal length
  void
  set_focal_length( const double& focal_length )
  {
    focal_length_ = focal_length;
  }
  ...
  void
  set_aspect_ratio( const double& aspect_ratio )
  {
    aspect_ratio_ = aspect_ratio;
  }
  ...
  void
  set_dist_coeffs( const vector_t& d )
  {
    dist_coeffs_ = d;
    max_distort_radius_sq_ = compute_max_distort_radius_sq();
  }
