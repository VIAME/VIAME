...
// Pybind wont allow track_descriptor_sets to be passed around with
// smart_pointers
// We'll make a smart_ptr to a copy of the track_descriptor_set passed in
void
database_query_set_descriptors(
  kv::database_query& self,
  const kv::track_descriptor_set& tdset )
{
  // Note that we're making a copy of a vector of pointers.
  // Elements of the vector can still be modified by reference on the python
  // side
...
PYBIND11_MODULE( database_query, m )
{
  ...
  py::class_< kv::database_query, std::shared_ptr< kv::database_query > >(
    ...
    .def_property(
      "type", &kv::database_query::type,
      &kv::database_query::set_type )
    .def_property(
      "temporal_filter", &kv::database_query::temporal_filter,
      &kv::database_query::set_temporal_filter )
    .def_property(
      "spatial_filter", &kv::database_query::spatial_filter,
      &kv::database_query::set_spatial_filter )
    .def_property(
      "spatial_region", &kv::database_query::spatial_region,
      &kv::database_query::set_spatial_region )
    .def_property(
      "stream_filter", &kv::database_query::stream_filter,
      &kv::database_query::set_stream_filter )
    .def_property(
      "descriptors", &database_query_get_descriptors,
      &database_query_set_descriptors )
    .def_property(
      "threshold", &kv::database_query::threshold,
      &kv::database_query::set_threshold )
