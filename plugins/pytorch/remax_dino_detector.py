...
class ReMaxDINODetector(ImageObjectDetector):
    ...
    def detect(self, image_data):
        ...
        output = self.postprocessors["bbox"](output, torch.Tensor([[1.0, 1.0]]).cuda())[
            0
        ]
        ...
        for i in range(len(pred_label)):
            ...
            if label not in test_feats.keys():
                ...
                test_feats[label] = torch.cat(
                    (test_feats[label], feats.unsqueeze(0)), 0
                )
        prob_test = []
        test_data = []
        ...
        for bbox, score, novelty_prob, label in zip(bboxes, scores, prob_test, labels):
            ...
            new_bbox = torch.tensor(
                [
                    bbox[0] * inputs.shape[2],
                    bbox[1] * inputs.shape[3],
                    bbox[2] * inputs.shape[2],
                    bbox[3] * inputs.shape[3],
                ]
            )
            ...
            detected_object = DetectedObject(
                bounding_box, np.max(class_confidence), detected_object_type
            )
