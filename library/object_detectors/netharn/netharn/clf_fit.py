...
from viame.object_detectors.netharn.netharn import clf_dataset
...
class ClfModel(nh.layers.Module):
    ...
    def _init_backbone(self, key='url'):
        ...
        from viame.object_detectors.netharn.netharn.initializers.functional import load_partial_state
        if key == 'url':
...
class ClfHarn(nh.FitHarn):
    ...
    def on_epoch(harn):
        ...
        # from viame.object_detectors.netharn.netharn.metrics import confusion_vectors
...
def setup_harn(cmdline=True, **kw):
    ...
    hyper = nh.HyperParams(
        ...
        xpu=nh.XPU.coerce(config['xpu'], p2p=config['xpu_p2p']),
