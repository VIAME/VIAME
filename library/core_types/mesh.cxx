...
::approx_equal( mesh_face_array_base const& other, double epsilon ) const
{
  ...
  const mesh_face_array other_actual = *std::make_shared< mesh_face_array >(
    other );
  for( unsigned int i = 0; i < size(); i++ )
  {
...
::build_from_ifs( const std::vector< std::vector< unsigned int > >& face_list )
{
  ...
  const unsigned int num_faces =
    static_cast< unsigned int >( face_list.size() );
...
::compute_vertex_normals()
{
  ...
  for( unsigned int he = 0; he < half_edges.size(); ++he )
  {
    ...
    normals[ v ] +=
      mesh_tri_normal( verts[ v ], verts[ vn ], verts[ vp ] ).normalized();
  }
...
::label_ccw_tex_faces_valid()
{
  switch( tex_coord_status_ )
  {
    ...
    {
      logger_handle_t logger( get_logger( "viame.mesh" ) );
      LOG_ERROR(
