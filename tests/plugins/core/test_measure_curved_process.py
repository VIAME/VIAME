"""Centerline transfer and curved measurement without kwiver or a stereo model."""
...
def module(monkeypatch):
    for name in ('kwiver', 'kwiver.sprokit', 'kwiver.vital', 'viame', 'viame.core'):
        monkeypatch.setitem(sys.modules, name, types.ModuleType(name))
    pipeline = types.ModuleType('kwiver.sprokit.pipeline')
    pipeline.datum, pipeline.process = Mock(), Mock()
    kwiver_process = types.ModuleType('kwiver.sprokit.processes.kwiver_process')
    kwiver_process.KwiverProcess = type('KwiverProcess', (), {})
    ...
    monkeypatch.setitem(sys.modules, 'kwiver.sprokit.pipeline', pipeline)
    monkeypatch.setitem(sys.modules, 'kwiver.sprokit.processes', types.ModuleType('kwiver.sprokit.processes'))
    monkeypatch.setitem(sys.modules, 'kwiver.sprokit.processes.kwiver_process', kwiver_process)
