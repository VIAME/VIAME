...
::run()
{
  ...
  // Note that you have to determine that the options is there before getting
  // value
  // Seg-fault otherwise
  std::string opt_app_version =
    cmd_args.count( "version" ) ? cmd_args[ "version" ].as< std::string >()
                                : "";
  std::string opt_install_prefix =
    cmd_args.count( "prefix" ) ? cmd_args[ "prefix" ].as< std::string >() : "";
