...
"""`viame.colmap`, kept so that its old module paths still import.

the `colmap` plugin was dissolved into the libraries in `library/` in phase 2.
...
`design/scripts/build_compat_aliases.py` -- so `import viame.colmap.x`,
`from viame.colmap.x import Y` and `python -m viame.colmap.x` all reach
