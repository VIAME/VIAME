...
def _require_re(package):
    return re.compile(r'VIAME_PYTHON_REQUIRE\(\s*"'
                      + re.escape(package) + r'\.(\w+)"\s*\)')


def _init_re(package):
    return re.compile(r'^from ' + re.escape(package) + r'\.(\w+) import \*', re.M)
...
def collect(source_dir, package):
    """{module: {dependency, ...}}.
    ...
    for path in sorted(Path(source_dir).glob("*_python.cxx")):
        ...
        modules[m.group(1)] = set(_require_re(package).findall(text))
...
def order(modules, preferred):
    ...
    while pending:
        ...
        if not ready:
            ...
            raise SystemExit(
                f"generate_python_fold: dependency cycle among: {cycle}\n"
...
//
// The one extension module behind `%(package)s`. Each translation unit that
// was its own module registers into a submodule of the same name, and
// the package's `<name>.py` re-exports it, so `%(package)s.<name>` still
// names what it always did.
//
// The order below is a topological sort of the `VIAME_PYTHON_REQUIRE` lines
// in the bindings plus the base classes in their `py::class_` arguments,
// not the order the package's `__init__` imports in. Every submodule registers during
...
PYBIND11_MODULE( %(module)s, m )
{
  m.doc() = "%(package)s as one extension module. Reach the submodules "
            "through %(package)s.<name>, which is what re-exports them.";

%(calls)s}
'''


def render(names, module, package):
...
def main(argv=None):
    ...
    p.add_argument("--package", required=True,
                   help="the python package, e.g. viame.types")
    p.add_argument("--module", required=True,
                   help="the folded extension module's name, e.g. _types")
    p.add_argument("--init",
                   help="a file whose `from <package>.x import *` order breaks "
                        "ties, so the generated file stays stable")
    p.add_argument("--output", required=True)
    p.add_argument("--min-modules", type=int, default=2,
                   help="fail below this many, so a regex that quietly stops "
                        "matching is caught here and not at import. Callers "
                        "pass the count they expect; the default only catches "
                        "a fold that found nothing at all.")
    args = p.parse_args(argv)

    modules = collect(args.source_dir, args.package)
    if len(modules) < args.min_modules:
        raise SystemExit(f"generate_python_fold: found {len(modules)} modules in "
                         f"{args.source_dir}, expected at least {args.min_modules}")

    init = Path(args.init) if args.init else None
    preferred = (_init_re(args.package).findall(init.read_text())
                 if init and init.is_file() else [])
    ...
    text = render(names, args.module, args.package)
    # Do not rewrite an identical file: the timestamp would rebuild the module
    # on every configure.
    if not out.is_file() or out.read_text() != text:
        out.write_text(text)
    print(f"  {args.package}: {len(names)} modules folded into one")
    return 0
