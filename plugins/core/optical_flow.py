...
class OpticalFlowFilter( ImageFilter ):
  ...
  def filter( self, in_img ):
    ...
    if self.normalize == "adaptive":
      full_scale = max( float( np.percentile( mag, self.adaptive_percentile ) ),
                        self.adaptive_min_scale )

    if self.output == "vector":
      encoded = np.clip( 128.0 + flow * ( 127.0 / full_scale ), 0, 255 ).astype( np.uint8 )
    else:
...
def __vital_algorithm_register__():
  ...
  implementation_name = "ocv_optical_flow"
  if algorithm_factory.has_algorithm_impl_name(
      OpticalFlowFilter.static_type_name(), implementation_name ):
    return

  algorithm_factory.add_algorithm( implementation_name,
    "Dense Farneback optical-flow image filter", OpticalFlowFilter )
