...
namespace kwiver
{
  ...
  sprokit::process::type_t const kw_archive_writer_process::kwiver_corner_points( "kwiver:corner_points" );
...
public:
  ...
  static sprokit::process::port_t port_timestamp;
  static sprokit::process::port_t port_image;
  static sprokit::process::port_t port_src_to_ref_homography;
  static sprokit::process::port_t port_corner_points;
  static sprokit::process::port_t port_gsd;


  // Configuration values
  std::string m_output_directory;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_output_directory;
  static config::value_t const kw_archive_writer_process::priv::default_output_directory;

  std::string m_base_filename;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_base_filename;
  static config::value_t const kw_archive_writer_process::priv::default_base_filename;

  bool m_separate_meta;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_separate_meta;
  static config::value_t const kw_archive_writer_process::priv::default_separate_meta;

  std::string m_mission_id;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_mission_id;
  static config::value_t const kw_archive_writer_process::priv::default_mission_id;

  std::string m_stream_id;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_stream_id;
  static config::value_t const kw_archive_writer_process::priv::default_stream_id;

  bool m_compress_image;
  static sprokit::config::key_t const kw_archive_writer_process::priv::config_compress_image;
  static config::value_t const kw_archive_writer_process::priv::default_compress_image;
...
typedef kw_archive_writer_process::priv priv_t;
...
sprokit::process::port_t const priv_t::port_timestamp = sprokit::port_t("timestamp");
sprokit::process::port_t const priv_t::port_image = sprokit::port_t("image");
sprokit::process::port_t const priv_t::port_src_to_ref_homography = sprokit::port_t("src_to_ref_homography");
sprokit::process::port_t const priv_t::port_corner_points = sprokit::port_t("corner_points");
sprokit::process::port_t const priv_t::port_gsd = sprokit::port_t("gsd");
...
::_configure()
{
  ...
  d->m_output_directory = config_get< std::string > ( priv_t::config_output_directory );
  d->m_base_filename    = config_get< std::string > ( priv_t::config_base_filename );
  d->m_separate_meta    = config_get< bool > ( priv_t::config_separate_meta );
  d->m_mission_id       = config_get< std::string > ( priv_t::config_mission_id );
  d->m_stream_id        = config_get< std::string > ( priv_t::config_mission_id );
  d->m_compress_image   = config_get< bool > ( priv_t::config_compress_image );
...
::_init()
{
  ...
  if ( d->m_separate_meta )
  {
    ...
    d->m_meta_bstream = new vsl_b_ostream( this->meta_stream_ );
    if ( ! *d->m_meta_stream )
    {
  ...
  d->m_data_stream_ = new std::ofstream( data_filename.c_str(),
             std::ios::out | std::ios::trunc | std::ios::binary );
  this->data_bstream_ = new vsl_b_ostream( d->m_data_stream );
  if ( ! *d->m_data_stream )
  {
...
::_step()
{
  ...
  // gsd
  priv::gst_t gsd = grab_from_port_as< priv::gst_t > ( priv::port_gsd );
  ...
  ::make_config()
{
  ...
  declare_configuration_key(
    ...
    config::description_t( "Output directory where KWA will be written" ));

  declare_configuration_key(
    priv::config_base_filename,
    priv::default_base_filename,
    config::description_t( "Base file name (no extension) for KWA component files" ));

  declare_configuration_key(
    priv::config_separate_meta,
    priv::default_separate_meta,
    config::description_t( "Whether to write separate .meta file" ));

  declare_configuration_key(
    priv::config_mission_id,
    priv::default_mission_id,
    config::description_t( "Mission ID to store in archive" ));

  declare_configuration_key(
    priv::config_stream_id,
    priv::default_stream_id,
    config::description_t( "Stream ID to store in archive" ));

  declare_configuration_key(
    priv::config_compress_image,
    priv::default_compress_image,
    config::description_t( "Whether to compress image data stored in archive" ));
}
...
::write_frame_data(vsl_b_ostream& stream,
                   ...
                   corner_points_t const& corner_pts,
                   maptk::image const& img,
                   maptk::f2f_homography const& s2r_homog,
                   double gsd)
{
  int64_t u_seconds = static_cast< int64_t > ( ts.time() );
  ...
  // convert homography
  maptk::homography matrix const& ( s2r_homog );  // upcast to base matrix
