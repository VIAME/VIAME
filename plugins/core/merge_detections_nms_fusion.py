...
class MergeDetectionsNMSFusion(MergeDetections):
    ...
    def set_configuration(self, cfg_in):
        ...
        if self._rescore_model_file:
            self._rescore_model = dfc.load_rescore_model(self._rescore_model_file)
    ...
    def merge(self, det_sets):
        ...
        class_count = max(self._label_dic.values()) + 1 if self._label_dic else 0
        ...
        for model_ind, det_set in enumerate(det_sets):
            ...
            for det in det_set:
                ...
                if self._fuse_masks and det.mask is not None:
                    mask_list.append(np.squeeze(np.asarray(det.mask.image())))
                else:
        ...
        if self._width == 1 and len(all_boxes) > 0:
            ...
            for box_list in boxes_list:
                box_list[:] = [
                    [x[0] / norm_width, x[1], x[2] / norm_width, x[3]] for x in box_list
                ]
        if self._height == 1 and len(all_boxes) > 0:
            norm_height = max([x[3] for x in all_boxes]) + 1
            for box_list in boxes_list:
                box_list[:] = [
                    [x[0], x[1] / norm_height, x[2], x[3] / norm_height]
                    for x in box_list
                ]
        ...
        if self._fusion_type == "proben":
            np_boxes = [np.array(b, dtype=float).reshape(-1, 4) for b in boxes_list]
            dist_arrays = [dfc.dists_to_array(d, class_count) for d in dists_list]
            ...
            if need_contribs:
                contributors = dfc.find_contributors(
                    out_boxes, out_labels, boxes_list, labels_list, self._iou_thr
                )
        ...
        if self._rescore_model is not None and contributors is not None:
            for i in range(len(out_scores)):
                features = dfc.compute_stack_features(
                    out_scores[i], contributors[i], scores_list, n_models
                )
                out_scores[i] = dfc.logistic_predict(features, self._rescore_model)
        ...
        for i in range(len(out_boxes)):
            ...
            if fused_dists is not None:
                ...
                for class_id in range(class_count):
                    if class_id == dfc.BACKGROUND_ID or not class_id in self._id_dic:
                        continue
