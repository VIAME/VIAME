...
"""`viame.pytorch`, kept so that its old module paths still import.

the `pytorch` plugin was dissolved into the libraries in `library/` in phase 2.
...
`design/scripts/build_compat_aliases.py` -- so `import viame.pytorch.x`,
`from viame.pytorch.x import Y` and `python -m viame.pytorch.x` all reach
