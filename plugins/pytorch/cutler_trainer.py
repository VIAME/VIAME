...
class ConvNextCascadeRCNNTrainer(TrainDetector):
    ...
    _options = [
        ...
        _Option(
            "_launcher", "launcher", "pytorch", str, ""
        ),  # "none, pytorch, slurm, or mpi"
    ...
    def set_configuration(self, cfg_in):
        ...
        if self._seed_weights:
            ...
            "checkpoint_override" in self.config and self.config["checkpoint_override"]
        ):
        ...
        if config_file["name"] == "CutLER/pretrain_algo.py":
            if os.path.exists(
                os.path.join(str(self.config["work_dir"]), "pretrain.pth")
            ):
                self.original_chkpt_file = os.path.join(
                    self.config["work_dir"], "pretrain.pth"
                )
    ...
    def set_mmdet_config(self):
        ...
        self.mmdet_config_file = os.path.join(
            self.config_dir, os.path.basename(self.config["mmdet_model_config_file"])
        )
        mmdet_config = mmcv.Config.fromfile(self.mmdet_config_file)
        mmdet_config.dataset_type = "CocoDataset"

        mmdet_config.data_root = None
        mmdet_config.data.ann_file = str(
            os.path.join(self.config["work_dir"], "train_data_coco.json")
        )
        ...
        if mmdet_config.data.train.type == "RepeatDataset":
            ...
            if (
                self.config["use_class_balanced"]
                and mmdet_config.data.train.dataset.type != "ClassBalancedDataset"
            ):
                mmdet_config.data.train.dataset.type = "CocoDataset"
                mmdet_config.data.train.dataset.data_root = None
                mmdet_config.data.train.dataset.ann_file = str(
                    os.path.join(self.config["work_dir"], "train_data_coco.json")
                )
                ...
                mmdet_config.data.train.dataset.dataset.ann_file = str(
                    os.path.join(self.config["work_dir"], "train_data_coco.json")
                )
                ...
                mmdet_config.data.train.dataset.type = "CocoDataset"
                mmdet_config.data.train.dataset.data_root = None
                mmdet_config.data.train.dataset.ann_file = os.path.join(
                    self.config["work_dir"], "train_data_coco.json"
                )
                mmdet_config.data.train.dataset.img_prefix = self.image_root
                mmdet_config.data.train.dataset.classes = mmdet_config.classes
        elif (
            mmdet_config.data.train.type == "ClassBalancedDataset"
            and self.config["use_class_balanced"]
        ):
            ...
            mmdet_config.data.train.ann_file = str(
                os.path.join(self.config["work_dir"], "train_data_coco.json")
            )
            mmdet_config.data.train.img_prefix = self.image_root
            mmdet_config.data.train.classes = (
                mmdet_config.classes
            )  # tuple(train_dataset.category_to_category_index.values())
        ...
        if self.stage == "adapt":
            if (
                "iters_per_ckpt_adapt" in self.config
                and len(self.config["iters_per_ckpt_adapt"]) > 0
                and self.ckpt < len(self.config["iters_per_ckpt_adapt"])
            ):
            ...
            if (
                "iters_per_ckpt" in self.config
                and len(self.config["iters_per_ckpt"]) > 0
                and self.ckpt < len(self.config["iters_per_ckpt"])
            ):
                num_iter = self.config["iters_per_ckpt"][self.ckpt]
            else:
                num_iter = self.config["max_iters"]

        mmdet_config.lr_config.warmup_iters = (
            1
            if self.config["warmup_iters"] >= num_iter
            else self.config["warmup_iters"]
        )
        mmdet_config.lr_config.step = [
            step for step in self.config["lr_steps"] if step < num_iter
        ]
    ...
    def update_model(self):
        ...
        checkpoint_file = os.path.join(
            self.config_dir, os.path.basename(self.original_chkpt_file)
        )
        ...
        if (
            "save_model_every_ckpt" in self.config
            and self.config["save_model_every_ckpt"]
        ):
            if os.path.exists(os.path.join(self.config["work_dir"], "latest.pth")):
                fname = str(self.stage) + "_" + str(self.ckpt) + "_model.pth"
                shutil.copy(
                    os.path.join(self.config["work_dir"], "latest.pth"),
                    os.path.join(self.config["work_dir"], fname),
                )
        ...
        if "eval_train_set" in self.config and self.config["eval_train_set"]:
            if os.path.exists(os.path.join(self.work_dir, "latest.pth")):
                fname = str(self.stage) + "_" + str(self.ckpt) + "_model.pth"
                shutil.copy(
                    os.path.join(self.work_dir, "latest.pth"),
            ...
            if os.path.exists(os.path.join(self.work_dir, "train_data_coco.json")):
                shutil.copy(
                    os.path.join(self.work_dir, "train_data_coco.json"),
                    os.path.join(self.work_dir, fname),
                )
