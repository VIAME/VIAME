...
vital_descriptors_to_ocv( const std::vector< vital::descriptor_sptr >& desc )
{
  ...
  for( size_t i = 0; i < num; ++i )
  {
    ...
    if( !d || d->size() != dim )
    {
      VITAL_THROW(
        vital::invalid_value,
        "mismatch type or size when converting descriptors to OpenCV" );
    }
...
#define CONVERT_CASE( T )                                               \
  ...
  APPLY_TO_TYPES( CONVERT_CASE );
    default:
      VITAL_THROW(
        vital::invalid_value, "No case to handle OpenCV descriptors of type " +
    ...
    break;
           ...
           APPLY_TO_TYPES( CONVERT_CASE );
             default:
               VITAL_THROW(
                 vital::invalid_value,
                 "No case to handle OpenCV descriptors of type " +
                 cv_type_to_string( data_.type() ) );
           }
#undef CONVERT_CASE
           return d_sptr;
