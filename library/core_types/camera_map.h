...
public:
  ...
  /// Return a map from integer IDs to camera shared pointers
  virtual map_camera_t
  cameras() const { return data_; }
...
template < class T > class camera_map_of_;
...
public:
  ...
  //@{
  void
  insert( frame_id_t fid, std::shared_ptr< T > const& cam )
  {
    data_[ fid ] = cam;
  }

  void
  insert( frame_id_t fid, std::shared_ptr< T >&& cam )
  {
  ...
  void
  set_from_base_cams( camera_map_sptr base_cams )
  {
  ...
  void
  set_from_base_camera_map( const camera_map::map_camera_t& base_cams_map )
  {
    clear();
  ...
  clone()
  {
    ...
    for( auto& d : data_ )
    {
      the_clone->insert(
        d.first,
        std::static_pointer_cast< T >( d.second->clone() ) );
    }
    return the_clone;
  }
  ...
  template < typename B >
  std::map< frame_id_t, std::shared_ptr< B > >
  map_of_()
  {
  ...
  virtual frame_to_T_sptr_map const&
  T_cameras() const { return data_; }
