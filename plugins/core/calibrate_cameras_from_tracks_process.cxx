...
public:
  ...
  static kv::feature_track_set_sptr merge_features_track(const kv::feature_track_set_sptr &feature_track1,
                                                         const kv::feature_track_set_sptr &feature_track2);

  static kv::landmark_map_sptr
  merge_landmarks_map(const kv::landmark_map_sptr &landmarks_map1, const kv::landmark_map_sptr &landmarks_map2);
...
calibrate_cameras_from_tracks_process::priv::get_attribute_value(const std::string &note) {
...
calibrate_cameras_from_tracks_process::priv::merge_features_track(const kv::feature_track_set_sptr &feature_track1,
                                                                  const kv::feature_track_set_sptr &feature_track2) {
...
calibrate_cameras_from_tracks_process::priv::merge_landmarks_map(const kv::landmark_map_sptr &landmarks_map1,
...
std::pair<kv::feature_track_set_sptr, kv::landmark_map_sptr>
calibrate_cameras_from_tracks_process::priv::split_object_track(const kv::object_track_set_sptr &object_track) {
  ...
  auto is_erased = [&erase_id_set](size_t track_id) { return erase_id_set.find(track_id) != std::end(erase_id_set); };
  ...
  LOG_DEBUG(m_logger, "Landmarks number splitted from object track : " << landmarks.size());
...
calibrate_cameras_from_tracks_process::priv::~priv() {
}


// =============================================================================
calibrate_cameras_from_tracks_process::calibrate_cameras_from_tracks_process(kv::config_block_sptr const &config)
    : process(config), d(new calibrate_cameras_from_tracks_process::priv(this)) {
  make_ports();
  make_config();
...
calibrate_cameras_from_tracks_process::~calibrate_cameras_from_tracks_process() {
}


// -----------------------------------------------------------------------------
void calibrate_cameras_from_tracks_process::make_ports() {
...
void calibrate_cameras_from_tracks_process::_step() {
  ...
  kv::camera_map_sptr cameras_map = std::make_shared<kv::simple_camera_map>(cameras);
  ...
  if (t1_size != t2_size) {
    std::stringstream ss;
    ss << "Track size incoherent between left and right camera : " << t1_size << " - " << t2_size;
