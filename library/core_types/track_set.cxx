...
void
track_set_implementation
::notify_new_state( [[maybe_unused]] track_state_sptr ts )
{
...
void
track_set_implementation
::notify_removed_state( [[maybe_unused]] track_state_sptr ts )
{
  // by default, notification does nothing
}
...
::merge_tracks( track_sptr t1, track_sptr t2 )
{
  ...
  while( t1 && t1->empty() &&
         ( tdr =
             std::dynamic_pointer_cast< track_data_redirect >( t1->data() ) ) )
  {
    t1 = tdr->redirect_track;
  }
  while( t2 && t2->empty() &&
         ( tdr =
             std::dynamic_pointer_cast< track_data_redirect >( t2->data() ) ) )
  {
...
::clone( clone_type ct ) const
{
  std::unique_ptr< simple_track_set_implementation > new_stsi =
    std::unique_ptr< simple_track_set_implementation >(
      new simple_track_set_implementation() );
