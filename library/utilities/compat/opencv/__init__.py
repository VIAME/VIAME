...
"""`viame.opencv`, kept so that its old module paths still import.

the `opencv` plugin was dissolved into the libraries in `library/` in phase 2.
...
`design/scripts/build_compat_aliases.py` -- so `import viame.opencv.x`,
`from viame.opencv.x import Y` and `python -m viame.opencv.x` all reach
