...
class ReMaxDINOTrainer(TrainDetector):
    ...
    def update_model(self):
        self.dino_config.coco_path = str(
            os.path.join(self._work_dir, "train_data_coco.json")
        )  # the path of coco
        ...
        if self._debug_mode and os.path.exists(self._feature_cache):
            ...
            for image, targets in dataset_train:
                ...
                idxs_true, idxs_pred, _, _ = self.match_bboxes(
                    targets["boxes"], pred_boxes
                )
                for i in range(idxs_pred.size):
                    ...
                    if label not in train_feats.keys():
                        ...
                        train_feats[label] = torch.cat(
                            (train_feats[label], feats.unsqueeze(0)), 0
                        )
                ...
                for i in range(len(pred_label)):
                    if i not in idxs_pred:
                        if "-1" not in train_feats.keys():
                            ...
                            train_feats["-1"] = torch.cat(
                                (train_feats["-1"], pred_feats[i].unsqueeze(0)), 0
                            )
