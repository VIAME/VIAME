  ...
  switch( traits.type )
  {
    ...
    case pixel_traits::pixel_type::UNSIGNED:
      switch( traits.num_bytes )
      {
        ...
        default:
          throw std::runtime_error(
            "Unsigned must have itemsize 1, 2, 4, or 8, not " +
    ...
    case pixel_traits::pixel_type::SIGNED:
      switch( traits.num_bytes )
      {
        ...
        default:
          throw std::runtime_error(
            "Signed must have itemsize 1, 2, 4, or 8, not " +
            std::to_string( itemsize ) );
      }
    case pixel_traits::pixel_type::FLOAT:
      switch( traits.num_bytes )
      {
        ...
        default:
          throw std::runtime_error(
            "Float must have itemsize 2, 4, or 8, not " +
            std::to_string( itemsize ) );
      }
...
::asarray( image_t img )
{
  ...
  switch( pixel_type )
  {
    ...
    default:
      throw std::runtime_error(
        "Unidentified pixel trait and number of bytes encoutered" );
  }
}
...
::image( py::module& m )
{
  ...
  py::enum_< pixel_traits::pixel_type >( img, "Types" ).value(
    "PIXEL_UNKNOWN",
    pixel_traits::pixel_type::UNKNOWN ).value(
      "PIXEL_BOOL",
      pixel_traits::pixel_type::BOOL ).value(
      "PIXEL_UNSIGNED",
      pixel_traits::pixel_type::UNSIGNED ).value(
      "PIXEL_SIGNED",
      pixel_traits::pixel_type::SIGNED ).value(
      "PIXEL_FLOAT",
