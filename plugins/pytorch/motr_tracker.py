...
def build_track_query_model(
    d_model=256, nhead=8, num_layers=3, crop_size=64, dropout=0.1
):
    """
    ...
    class TrackQueryModel(nn.Module):
        ...
        def decode_queries(self, queries, det_tokens):
            ...
            if det_tokens.shape[0] == 0:
                return queries
            return self.decoder(queries.unsqueeze(0), det_tokens.unsqueeze(0)).squeeze(
                0
            )

        def association_logits(
            self, queries, query_boxes_norm, det_tokens, det_boxes_norm, dt
        ):
            """
        ...
        def update_query(self, query, det_token):
            ...
            return self.query_update(
                det_token.unsqueeze(0), query.unsqueeze(0)
            ).squeeze(0)
...
class MOTRTrackerConfig(scfg.DataConfig):
    ...
    match_thresh = scfg.Value(
        0.5, help="Minimum association probability to accept a match"
    )
    new_track_thresh = scfg.Value(
        0.6, help="Minimum detection confidence to start a new track"
    )
...
class _QueryTrack:
    ...
    def __init__(self, query, box_tlbr, score, frame_id, timestamp, detected_object):
...
class MOTRTracker(TrackObjects):
    ...
    def set_configuration(self, cfg_in):
        ...
        if c.model_path and os.path.exists(c.model_path):
            checkpoint = torch.load(c.model_path, map_location="cpu", weights_only=True)
    ...
    def track(self, ts, image, detections):
        ...
        with torch.no_grad():
            ...
            if D > 0:
                crops = crop_detections(np_image, det_boxes, self._crop_size)
                crops_t = (
                    None if crops is None else torch.from_numpy(crops).to(self._device)
                )
                boxes_norm = boxes_tlbr_to_norm_cxcywh(det_boxes, img_w, img_h)
                det_boxes_norm_t = torch.from_numpy(boxes_norm).to(self._device)
            ...
            if T > 0 and D > 0:
                ...
                track_boxes_norm_t = torch.from_numpy(track_boxes_norm).to(self._device)
                dt = torch.tensor(
                ...
                logits = self._model.association_logits(
                    refined, track_boxes_norm_t, det_tokens, det_boxes_norm_t, dt
                )
                ...
                for r, col in matched_track.items():
                    trk = self._tracks[r]
                    trk.query = self._model.update_query(refined[r], det_tokens[col])
        ...
        # Drop dead tracks
        self._tracks = [
            t for t in self._tracks if t.time_since_update <= c.track_buffer
        ]

        output = [
            t
            for t in self._tracks
            if t.time_since_update == 0 and t.hits >= c.min_hits and len(t.history) > 0
        ]
