...
def shortest_unique_prefixes(items, sep=None, allow_simple=True, min_length=0, allow_end=False):
    ...
    Args:
        ...
        allow_simple (bool): if True tries to construct a simple feasible
            solution before resorting to the optimal algorithm.
    ...
    if sep is None:
        token_lists = [tuple(item) for item in items]

        def join(tokens):
            return ''.join(tokens)
    else:
        # In some simple cases we can avoid the general algorithm
        if allow_simple:
        ...
        token_lists = [tuple(item.split(sep)) for item in items]

        def join(tokens):
            return sep.join(tokens)

    # A prefix is unique exactly when it is longer than what the item shares
    # with its lexicographic neighbors, so only those two need comparing.
    order = sorted(range(len(items)), key=token_lists.__getitem__)
    shared = [-1] * len(items)
    for rank, index in enumerate(order):
        toks = token_lists[index]
        for other_rank in (rank - 1, rank + 1):
            if 0 <= other_rank < len(order):
                other = token_lists[order[other_rank]]
                common = 0
                for tok1, tok2 in zip(toks, other):
                    if tok1 != tok2:
                        break
                    common += 1
                if common > shared[index]:
                    shared[index] = common

    unique = []
    for index, toks in enumerate(token_lists):
        size = min(shared[index] + 1, len(toks))
        prefix = join(toks[:size])
        while size < len(toks) and len(prefix) < min_length:
            size += 1
            prefix = join(toks[:size])
        if not allow_end and size <= shared[index]:
            freq = sum(toks[:size] == other[:size] for other in token_lists)
            raise AssertionError(
                'item={} has no unique prefix. freq={}'.format(
                    items[index], freq))
...
if __name__ == '__main__':
    r"""
    CommandLine:
        xdoctest -m netharn.util.util_fname all
    """
    import xdoctest
