...
"""`viame.onnx`, kept so that its old module paths still import.

the `onnx` plugin was dissolved into the libraries in `library/` in phase 2.
...
`design/scripts/build_compat_aliases.py` -- so `import viame.onnx.x`,
`from viame.onnx.x import Y` and `python -m viame.onnx.x` all reach
