...
PYBIND11_MODULE( geodesy, m )
{
  ...
  // The geo_conversion class itself isn't bound because of its protected
  // virtual destructor,
  // which pybind complains about. We'll just bind the important free functions
  // that wrap
  // the functor
  ...
  m.def(
    "geo_conv",
    ( ( kv::vector_2d ( * )(
      kv::vector_2d const&, int,
      int ) ) & kv::geo_conv ) );
  m.def(
    "geo_conv",
    ( ( kv::vector_3d ( * )(
      kv::vector_3d const&, int,
      int ) ) & kv::geo_conv ) );

  // utm_ups_zone_t
  py::class_< kv::utm_ups_zone_t, std::shared_ptr< kv::utm_ups_zone_t > >(
    m,
    "UTMUPSZone" )
    .def(
    py::init(
      ( kv::utm_ups_zone_t ( * )(
        double,
        double ) ) & kv::utm_ups_zone ) )
    .def(
      py::init(
        ( kv::utm_ups_zone_t ( * )(
          kv::vector_2d const& lon_lat ) ) & kv::utm_ups_zone ) )
    .def(
      py::init(
        ( kv::utm_ups_zone_t ( * )(
          kv::vector_3d const& lon_lat_alt ) ) & kv::utm_ups_zone ) )
