> This describes the former `onnx` plugin directory. Its detector and
> classifiers are `library/object_detectors/onnx` and
> `library/classifiers/onnx`, its stereo tools `library/measurement/onnx`.
