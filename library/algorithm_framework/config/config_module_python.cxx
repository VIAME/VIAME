...
#include "config_helpers_python.h"
...
PYBIND11_MODULE( _config, m )
{
  ...
  py::class_< kv::config_block, kv::config_block_sptr >(
    ...
    // , doc_Config_subblock
    ,
    R"pbdoc(Returns a :class:`viame.config.Config` from the
            configuration using the name of the subblock. The object is a copy
            of the block in the configuration.

            :param name: The name of the subblock in a :class:`viame.config.Config` object
            :return: a subblock of type :class:`viame.config.Config`
            )pbdoc"
    )
    .def(
      "subblock_view", &kv::config_block::subblock_view,
      py::arg( "name" ),
      R"pbdoc(Returns a :class:`viame.config.Config`
  ...
  m.def(
    ...
    R"pbdoc(Returns an empty :class:`viame.config.Config` object)pbdoc",
