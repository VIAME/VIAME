...
struct IdPair
{
  viame::track_id_t left_id;
  viame::track_id_t right_id;
};

struct VIAME_MEASUREMENT_EXPORT Pairing
{
  std::set< viame::frame_id_t > frame_set;
...
public:
  ...
  // Tracks status memo
  std::map< viame::track_id_t, viame::track_sptr > m_tracks_with_3d_left, m_right_tracks_memo;
  ...
  update_left_tracks_3d_position( const std::vector< viame::track_sptr >& tracks,
  ...
  keep_right_tracks_in_current_frame( const std::vector< viame::track_sptr >& tracks,
                                      const viame::timestamp& timestamp );

  /// @brief returns last track id available in both left and right track map
  viame::track_id_t last_left_right_track_id() const;
  ...
  void pair_left_right_tracks( const std::vector< viame::track_sptr >& left_tracks,
  ...
  void append_paired_frame( const viame::track_sptr& left_track,
  ...
  split_paired_tracks_to_new_tracks( std::vector< viame::track_sptr >& left_tracks,
  ...
  struct Range
  {
    viame::track_id_t left_id, right_id, new_track_id;
    viame::frame_id_t frame_id_first, frame_id_last;
    int detection_count;
  };
