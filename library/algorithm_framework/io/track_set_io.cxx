...
read_track_file( path_t const& file_path )
{
  ...
  {
    VITAL_THROW(
      file_not_found_exception, file_path,
  ...
  for( std::string line; std::getline( input_stream, line ); )
  {
    ...
    track_sptr t;
    std::map< track_id_t,
      track_sptr >::const_iterator it = track_map.find( tid );
    if( it == track_map.end() )
    {
      t = track::create();
      t->set_id( tid );
...
read_feature_track_file( path_t const& file_path )
{
  ...
  for( std::string line; std::getline( input_stream, line ); )
  {
    ...
    {
      // dummy descriptor.
      ftsd->descriptor = std::make_shared< descriptor_fixed< unsigned char,
        1 > >();
