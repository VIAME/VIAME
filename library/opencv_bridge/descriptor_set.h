...
class VIAME_OPENCV_BRIDGE_EXPORT descriptor_set
...
public:
  ...
  virtual size_t
  size() const override { return data_.rows; }
  virtual bool
  empty() const override { return size() == 0; }
  ...
  const cv::Mat&
  ocv_desc_matrix() const { return data_; }
...
VIAME_OPENCV_BRIDGE_EXPORT cv::Mat
