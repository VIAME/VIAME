...
std::ostream&
operator<<( std::ostream& os, image_pixel_traits::pixel_type pt )
{
  switch( pt )
  {
...
std::ostream&
operator<<( std::ostream& os, image_pixel_traits const& pt )
{
...
/// Return true if the pixels accessible in this image form a contiguous memory
/// block
bool
image
::is_contiguous() const
{
  ...
  std::pair< ptrdiff_t, size_t > sizes[ 3 ] = { { w_step_, width_ },
    { h_step_, height_ }, { d_step_, depth_ } };
...
::copy_from( const image& other )
{
      ...
      this->is_contiguous() )
  {
    std::memcpy(
      data, o_data,
      width_ * height_ * depth_ * pixel_traits_.num_bytes );
    return;
  }
...
::set_size( size_t width, size_t height, size_t depth )
{
  ...
  data_ = image_memory_sptr(
    new image_memory(
      width * height * depth *
      pixel_traits_.num_bytes ) );
  width_ = width;
  height_ = height;
  depth_ = depth;
  first_pixel_ = data_->data();
...
{
  auto crop_first_pixel =
    reinterpret_cast< const char* >( this->first_pixel() );
  crop_first_pixel +=
