...
| P5-T05 | Remove the kwiver submodule | P5-T04 | in-progress | 0abda7256, d4626adb8 | In progress. Two steps landed so far. The `kwiver` python package -- 422 files -- is VIAME's `python/kwiver` now; the layout table's "bindings alongside the C++" cannot hold while the package is called `kwiver`, since 48 of the 67 files under `vital/types` share a name with the vital source they bind and each package level needs its own `__init__.py`, so they keep their tree until P11 (finding 1.13). And `arrows/` is gone entirely, along with the matlab, transport and vxl process directories: everything in it was either split across `library/` by P5-T04 or switched off by an earlier phase. What is left in the submodule is the CMake that builds `library/` and `python/kwiver`, kwiversys, the sprokit example and cluster processes and schedulers, the tool runner, kwiver's config files and its macro library. Finishing needs phase 1's single build (finding 2.2, P1-T05): while kwiver is configured separately nothing left in it can link a VIAME library, so the last pieces cannot move one at a time |
